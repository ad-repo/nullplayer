...
| B145 | **Three Miku-family skins render an entirely empty main player window.** Found by the 2026-09-06 corpus census: `Love is War Miku.wal` (456×419) and `Love Is War Miku V2.wal` (300×450) draw **0.0%** opaque pixels in `main/normal`, and `hatsune_miku_5_winamp_by_kaza_sou_d6izotp.wal` (514×313) draws 2.2%. This is specific to the main window and to these three archives: every other window each skin declares — playlist, equalizer, library, video, notifier, preferences — renders 100% opaque in the same sweep, and the next-lowest main window in the corpus is Bio-Nid at 22.4%, which is a genuinely irregular silhouette. All three load clean, rate **complete** on the census (every surface has a home, no missing bitmaps) and read `level=degraded` — so nothing structural flags them, and only the author-screenshot comparison caught it. Reproduce with `scripts/wal_skin_census.sh` and read `png/<skin>/main-normal.png`, or `WINAMP_MODERN_WAL=<archive> WINAMP_MODERN_RENDER_DUMP=<dir>`. **They are the only skins in the corpus where a structurally complete rating and an empty window coexist**, which is the case the release rating in `docs/winamp-modern/skin-compatibility.md` exists to disclaim | 3 skins measured of 75; cause unknown, so corpus reach is unmeasured | M | Measured |
