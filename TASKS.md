...
| B81 | **Snap To Default cannot recover a `.wal` window.** `WindowManager.snapToDefaultPositions()` contains no Winamp Modern handling at all — it repositions the classic/original stack through the old per-feature controllers (`equalizerWindowController`, `playlistWindowController`, …), so a skin-owned auxiliary or hosted window that ends up off-screen has **no recovery path**; the main window is covered only because it happens to be `mainWindowController`. A borderless `.wal` window with its titlebar off the display cannot be dragged back either. Reported 2026-08-31 via Ebonite's media library. The plumbing already exists next door: the snap-target registry at `WindowManager.swift` builds records from `materializedAuxiliaryWindows` and `materializedHostedWindows` with `snapTarget: true`, and the snap routine simply does not consult it | 53 of 53 skins; 285 non-main containers ([M26]) | M | Live-reported |
...
| B83 | **`Layout.isVisible()` answers true for a window that is not on screen.** ClassicPro's drawer menu builds its *Widgets Manager* row as `addCommand("Widgets Manager", -3, getContainer("widgets.manager").getLayout("normal").isvisible(), 0)`, and the row comes back **ticked** with the window closed — measured in `RENDER_CLICK` on all five cPro skins (`*Widgets Manager#-3`). The same query drives the menu item's toggle branch, so the first pick can hide a window that was never shown | 5 skins measured; any skin querying a container's visibility | S | Live-reported |
...
- <a id="m26"></a>**M26:** `WINAMP_MODERN_WAL="$corpus_wal" WINAMP_MODERN_RENDER_DUMP=/tmp/c swift test --filter WinampModernRenderDumpTests`, then count the `main=false` entries on each `RENDER-DUMP containers: [...]` line. Measured 2026-08-31 over the 53 installed skins: **every one** declares at least one non-main container, **285** in total. Not all are windows the user can open, so this is an upper bound on exposure, not a count of stranded windows.
