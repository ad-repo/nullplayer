...
| Id | Item | Reach | Effort | Tier |
|---|---|---:|:---:|---|
| B126 | **cPro2's album-art pane comes up dead on *some* launches: no local cover, no File Info, and its selector menu does nothing — and a relaunch clears it.** Reported 2026-09-04, live, immediately after B124 landed. Observed twice, and **twice a plain relaunch fixed it**, which is the reporter's own reading ("maybe there is a loading bug with it"). Streaming cover art was arriving in the same session in which local art was not, so the pane and its holder are alive when it happens. **Not reproduced yet and not diagnosed** — the deciding evidence is a `WINAMP_MODERN_CALL_TRACE=1 WINAMP_MODERN_TRACE_MAKI=1` capture of a launch that *fails*; both captures so far are of launches that worked. Cheap discriminator when it next reproduces: does the classic skin show that track's local cover at the same moment? `tagviewer.m`'s `loadFileInfo()` opens `if(!scriptGroup.isVisible()) return;`, so a group reading invisible while on screen is the first thing to check, and `WinampModernHost.albumArtwork` drops any cover whose `NowPlayingManager.currentTrackId` does not match `engine.currentTrack?.id` — a load-order race there fits every symptom. **Not attributable to B124**: cPro2's cold-start script execution is byte-identical either side of it (265-program `RENDER_SCRIPTS` report, identical node list, only `fullScreen()` in the call-trace diff) | 1 skin reported; the artwork and holder paths are engine-wide | M | Live-reported |
| B125 | **`onShowLayout` fires at cold start only, and `onHideLayout` never fires at all**, so a shade↔normal round trip runs neither. `dispatchColdStartLayoutShown` is the only `onshowlayout` dispatch site in the codebase, and `WinampModernMainView.activateLayout` records the switch with `markLayoutRealized` and dispatches neither event. cPro2 therefore never calls `saveSkinPos()` on the way into shade nor re-runs `fullScreen()` on the way out, so the window geometry the skin saved is not restored. Found 2026-09-04 while closing B124, from the reporter's own "or repaint from shade?". The band itself survives a round trip — `fullScreen`'s writes are `setXmlParam`s that persist across a layout switch — so the visible symptom is geometry, not paint. **Needs live verification of the symptom before the dispatch is added**: announcing a layout the host is *leaving* reaches every skin with a shade mode, so it wants the corpus sweep | 1 skin measured; every skin with a shade layout and a layout script | M | Measured |
