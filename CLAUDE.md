...
- **KeychainHelper — login keychain + permissive ACL (no data-protection keychain)**: `KeychainHelper` uses the regular macOS login keychain (NOT `kSecUseDataProtectionKeychain`). The data-protection keychain requires a `keychain-access-groups` entitlement that ad-hoc signed apps (`codesign --sign -`) don't have, causing `-34018 errSecMissingEntitlement` on every write. Instead, `setDataKeychain` creates a permissive `SecAccess` ACL via `SecAccessCreate("NullPlayer", nil, &access)` — passing `nil` for trusted apps means any app can read without a prompt. This is required because ad-hoc signatures change on every rebuild; a strict ACL would prompt users on every app update. Dev builds (non-`.app` path) bypass keychain entirely and use UserDefaults. Do NOT add `kSecUseDataProtectionKeychain`, `kSecAttrAccessible`, `keychain-access-groups` entitlements, or any data-protection flags — they break DMG distribution.
