...
- **winamp-modern-skin-guide**: Winamp 5.x `.wal` skins — a slim router (pipeline, security model, file map, symptom→file routing table, section-title map) over `reference/` topic files: `loading`, `rendering`, `performance`, `scripting`, `components`, `classicpro`, `harness` (the canonical probe/env-var reference). Plus `compatibility.md`, `triage-playbook.md`, and per-skin state in `skins.md` → `skins/<skin>.md`
...
- **UI mode switching is live (no restart)**: `WindowManager.uiMode` is seeded once from `PlayerUIMode.stored()` and held in memory; its setter persists live changes in the full edition. `setClassicMode()`/`setModernMode()` call `WindowManager.reloadUI(toModernUI:)`, which tears down only the mode-dependent window layer (`teardownModeDependentWindows()`), flips the mode, prepares the target-mode runtime (`prepareUIRuntime`), reprograms the shared EQ nodes, rebuilds the menu, and recreates the windows. `AudioEngine` is owned by `WindowManager` (not a window), so playback/casting/playlist/seek survive untouched and audio state is deliberately not snapshotted. The mode-independent `VideoPlayerWindowController` is preserved across the switch. Skin-driven mode switches (`selectClassicSkin`/`selectModernSkin`/`loadDefaultClassicSkin`) are also live: they load the target skin into `currentSkin` (classic) or set `modernSkinName` (modern), then call `reloadUI`. Classic UI Size changes are also live now — `setUIScaleLevel(_:)` updates `uiScaleLevel` and `applyDoubleSize(previousScale:)` resizes in place (ending with a forced `forceRedrawTree` repaint so layer-backed classic views don't leave a stale "ghost"). `reloadUI` collapses `uiScaleLevel` to 1x before a mode switch and re-applies the captured level after, so nothing in the UI requires a relaunch. EQ initialization and live switches use `PlayerUIMode.usesModernEQLayout`, a distinct policy from `usesModernControllers` for downstream modes that combine custom controllers with the 21-band EQ. Forced-mode downstream editions are routed through `AppPersistence`: they ignore stale/default/`-uiMode` values, never persist the forced choice into shared preferences, and reject mismatched reload targets before controller teardown. `EditionPolicy.forcedUIMode` and `preferenceNamespace` must be supplied together; forcing without scoping can reintroduce cross-edition geometry through legacy unknown-mode decoding.
