# Creating NullPlayer Original Skins

This guide walks you through creating custom skins for NullPlayer's Original UI mode. The implementation is still named `ModernSkin` internally for compatibility. You can create a skin with nothing more than a text editor -- no programming required.
...
> **Original-Metal skins**: Original-Metal uses the modern skin engine, but it is a separate skin family. User-installed skins live in the compatibility directory `~/Library/Application Support/NullPlayer/MetalSkins/`, the current selection is stored under `metalSkinName`, and the built-in default is `Brushed Steel`.
...
3. Right-click the player, go to **Skins > Original**, and choose "My First Skin".

That's it. Every button, slider, label, and window border now uses your orange palette.

## Original-Metal Skins
...
3. Switch to the skin again from **Skins > Original-Metal** or reselect it from the menu to reload it.

### Built-in Metal Finishes

NullPlayer ships built-in Original-Metal skins, defined in code (not through `skin.json`) and selectable from **Skins → Original-Metal**:

| Finish | Look |
|--------|------|
| **Brushed Steel** (default) | Cool brushed steel faceplate |
| **Aluminum** | Bright polished/shiny silver |
| **Gunmetal** | Dark blue-gray satin |
| **Anodized Black** | Near-matte charcoal |
| **Brass** | Warm polished yellow-gold |
| **Bronze** | Dark muted brown-gold |
| **Copper** | Warm reddish-orange |

Each finish is a `MetalMaterial` preset (`Sources/NullPlayer/ModernSkin/MetalMaterial.swift`) that supplies every code-driven metal color: window sheen gradient + brushed stripes + accent strip, title bar, border, inset panels, the backlit-green LCD display, sliders, transport icons, the EQ panel/control/stroke + fader value ramp, and the spectrum analyzer bar gradient (`spectrumLow`/`spectrumHigh`). The `ModernSkin` carries the chosen material; the renderer and EQ view read colors from it instead of constants.
...
#### Metal Analyzer (vis_classic component)

The spectrum analyzer is a first-class part of every metal finish. Metal mode defaults **both** the main-window analyzer and the dedicated Spectrum window to the **vis_classic exact** core (a CPU port of the classic Winamp analyzer) running a per-finish profile named `Metal <finish>` — `Metal Brushed Steel`, `Metal Aluminum`, `Metal Gunmetal`, `Metal Anodized Black`, `Metal Brass`, `Metal Bronze`, `Metal Copper` — drawn **transparent** over the metal chrome (no opaque black box).

- The profiles are real `.ini` files in `Sources/NullPlayer/Resources/vis_classic/profiles/`. Each holds a 256-step `[BarColours]` ramp (quiet → loud) and a `[PeakColours]` ramp (loud → white). Colors are stored **`B G R`** per line, and the `Bar`/`Peak` ramps are generated by interpolating the finish's `spectrumLow → spectrumHigh` (bars) and `spectrumHigh → white` (peaks). The `[VolumeColours]`/`[VolumeFunction]` sections are finish-independent (copied verbatim across all metal profiles); only `Message` and the two color ramps differ between finishes.
- The matching **GPU Spectrum** mode and the small `ModernSkinRenderer` bars also read `spectrumLow`/`spectrumHigh`, so the analyzer looks consistent regardless of which mode is selected. The chrome `accentColor` is a dark metal tone and would render bars near-black, which is why the analyzer ramp is its own pair of material fields.
- The main window uses its own `.mainWindow`-scoped vis_classic bridge (`WindowManager.acquireMainWindowVisClassicBridge()`), independent of the dedicated Spectrum window's bridge, so the two windows keep separate profiles.
- The per-finish profile and the transparent background are **re-applied on every metal skin load and on switching finishes** (and via **Skins → Original-Metal → Reset Skin to Default**), so a stale non-metal profile or an opaque background left over from an earlier session can't survive into metal mode. This is wired in `ModernSkinLoader.createBuiltInMetalSkin` (the skin's `visualization.visClassic` config) and `ModernSkinEngine` (`forceProfileDefaults` for the metal family).

To change or add a built-in metal finish, update:

- `Sources/NullPlayer/ModernSkin/MetalMaterial.swift` — add/edit a preset (including `spectrumLow`/`spectrumHigh`)
...
- `Sources/NullPlayer/Resources/vis_classic/profiles/Metal <finish>.ini` — add the analyzer profile. Generate its `Bar`/`Peak` ramps from the finish's `spectrumLow`/`spectrumHigh` (B G R order), then copy the `[VolumeColours]`/`[VolumeFunction]` blocks verbatim from an existing `Metal *.ini`. The profile name **must** be exactly `Metal <finish>` to match `createBuiltInMetalSkin`'s `mainWindowProfile`/`spectrumWindowProfile`.
...
- pick a coherent base tint (cool steel, bright aluminum, gunmetal blue-gray, charcoal, brass/bronze/copper warm metals)
...
- pick `spectrumLow`/`spectrumHigh` (and the matching `.ini` ramp) to read against the chrome, not the dark `accent`
...
Other users can import it with **Skins > Original > Load Skin...** or drop the `.nsz` file into their `ModernSkins/` directory.
