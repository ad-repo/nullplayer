...
NullPlayer ships five built-in metal skins, defined in code (not through `skin.json`) and selectable from **Skins → Metal**:
...
- The per-finish profile and the transparent background are **re-applied on every metal skin load and on switching finishes** (and via **Skins → Metal → Reset Skin to Default**), so a stale non-metal profile or an opaque background left over from an earlier session can't survive into metal mode. This is wired in `ModernSkinLoader.createBuiltInMetalSkin` (the skin's `visualization.visClassic` config) and `ModernSkinEngine` (`forceProfileDefaults` for the metal family).
