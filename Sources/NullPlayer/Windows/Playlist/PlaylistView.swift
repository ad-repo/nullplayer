...
class PlaylistView: NSView {
    ...
    override func viewDidMoveToWindow() {
        ...
        playlistMarqueeLayer?.updateContentsScale(window?.backingScaleFactor ?? NSScreen.main?.backingScaleFactor ?? 2.0)
    ...
    @objc private func windowDidMiniaturize(_ notification: Notification) {
        ...
        stopDisplayTimer()
        playlistMarqueeLayer?.pauseAnimation()
    }
    ...
    @objc private func windowDidDeminiaturize(_ notification: Notification) {
        ...
        refreshTextBitmapCacheIfNeeded()
        startDisplayTimer()
        playlistMarqueeLayer?.resumeAnimation()
    }
    ...
    private func configurePlaylistMarqueeLayer(text: String, titleX: CGFloat, titleMaxWidth: CGFloat, rowRect: NSRect) -> Bool {
        guard !isShadeMode,
        ...
        guard rowRect.minY >= listTop && rowRect.maxY <= listBottom else { return false }
    ...
    private func setNeedsDisplayForListArea() {
        ...
        let hiddenTitleOffset = WindowManager.shared.hideTitleBars ? Layout.titleBarHeight : 0
        let listTop = max(0, Layout.titleBarHeight - hiddenTitleOffset)
        let listHeight = effectiveSize.height - Layout.titleBarHeight - Layout.bottomBarHeight
        let listRect = NSRect(
            x: 0,
            y: bounds.height - ((listTop + listHeight) * scale),
            width: bounds.width,
    ...
    private func drawTrackText(in context: CGContext, track: Track, index: Int, rect: NSRect, color: NSColor, font: NSFont, isCurrentTrack: Bool = false, isSelected: Bool = false) {
        ...
        if isCurrentTrack && titleWidth > titleMaxWidth && configurePlaylistMarqueeLayer(text: titleText, titleX: titleX, titleMaxWidth: titleMaxWidth, rowRect: rect) {
    ...
    func skinDidChange() {
        ...
        cacheTextBitmapCGImage()
        updateCurrentTrackTextWidth()
        playlistMarqueeLayer?.skinTextImage = WindowManager.shared.currentSkin?.text
        needsDisplay = true
    }
