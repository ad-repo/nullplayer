...
/// Controller for Jellyfin server configuration dialog
class JellyfinLinkSheet: NSWindowController, NSWindowDelegate {
    ...
    private var editingServer: JellyfinServer?
    var completionHandler: ((JellyfinServer?) -> Void)?
    ...
    convenience init(server: JellyfinServer?) {
        let w = NSWindow(
        ...
        w.title = server == nil ? "Add Jellyfin Server" : "Edit Jellyfin Server"
        ...
        if let s = server {
            ...
            if let creds = KeychainHelper.shared.getJellyfinServer(id: s.id) {
    ...
    private func setupUI() {
        ...
        nameField.placeholderString = "My Jellyfin Server"
    ...
    @objc private func test() {
        ...
        Task { @MainActor in
            do {
                ...
                _ = try await JellyfinServerClient.authenticate(
                    url: testURL,
        ...
        Task { @MainActor in
            do {
                ...
                if let e = editingServer {
                    try await JellyfinManager.shared.updateServer(id: e.id, name: nm, url: ur, username: us, password: pw)
                    server = JellyfinServer(id: e.id, name: nm, url: ur, username: us, userId: e.userId)
                } else {
                    server = try await JellyfinManager.shared.addServer(name: nm, url: ur, username: us, password: pw)
                }
...
/// Controller for Jellyfin server list dialog
class JellyfinServerListSheet: NSWindowController, NSWindowDelegate, NSTableViewDataSource, NSTableViewDelegate {
    
    private var tableView: NSTableView!
    private var servers: [JellyfinServer] = []
    ...
    private var linkSheet: JellyfinLinkSheet?
    ...
    convenience init() {
        ...
        w.title = "Manage Jellyfin Servers"
    ...
    private func load() {
        servers = JellyfinManager.shared.servers
    ...
    @objc private func edit() {
        ...
        linkSheet = JellyfinLinkSheet(server: servers[tableView.selectedRow])
    ...
    @objc private func remove() {
        ...
        if a.runModal() == .alertFirstButtonReturn {
            JellyfinManager.shared.removeServer(id: s.id)
            load()
        }
    }
    ...
    @objc private func connect() {
        ...
        Task { @MainActor in
            do {
                try await JellyfinManager.shared.connect(to: s)
                let handler = completionHandler
                completionHandler = nil
                window?.close()
                handler?(s)
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        case "name": cell.stringValue = (JellyfinManager.shared.currentServer?.id == s.id ? "● " : "") + s.name
