...
enum JellyfinRadioHistoryInterval: String, CaseIterable {
    case off = "off"
...
struct JellyfinRadioHistoryEntry {
    let id: Int64
    let trackId: String
    let title: String
    let artist: String?
...
// MARK: - JellyfinRadioHistory

class JellyfinRadioHistory {
    static let shared = JellyfinRadioHistory()

    private var db: Connection?

    // Table
    private let table = Table("jellyfin_radio_history")
    ...
    private func setupDatabase() {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            NSLog("JellyfinRadioHistory: Cannot locate Application Support directory")
            return
        }
        let dir = appSupport.appendingPathComponent("NullPlayer")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let dbPath = dir.appendingPathComponent("jellyfin_radio_history.db").path
    ...
    private func createTableIfNeeded(_ connection: Connection) throws {
        ...
        try connection.run(
            "CREATE INDEX IF NOT EXISTS idx_jellyfin_radio_history_played_at ON jellyfin_radio_history (played_at)"
        )
        try connection.run(
            "CREATE INDEX IF NOT EXISTS idx_jellyfin_radio_history_normalized_key ON jellyfin_radio_history (normalized_key)"
        )
    }
    ...
    func recordTrackPlayed(_ track: Track) {
        guard let db = db,
              let trackId = track.jellyfinId else { return }

        let serverId = track.jellyfinServerId ?? JellyfinManager.shared.currentServer?.id ?? ""
        let nKey = JellyfinRadioHistory.normalizedKey(title: track.title, artist: track.artist)
    ...
    func fetchHistory() -> [JellyfinRadioHistoryEntry] {
        guard let db = db else { return [] }
        do {
            let rows = try db.prepare(table.order(colPlayedAt.desc))
            return rows.map { row in
                JellyfinRadioHistoryEntry(
                    id: row[colId],
    ...
    func generateHistoryHTML() -> String {
        ...
        <title>Jellyfin Radio History</title>
        <style>
        ...
        function removeEntry(id) {
          fetch('/jellyfin-radio-history/delete/' + id, {method: 'POST'})
