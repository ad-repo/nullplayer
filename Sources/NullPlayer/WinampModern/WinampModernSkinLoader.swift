...
final class WinampModernSkinLoader {
    ...
    func load(from archiveURL: URL, additionalMounts: [WinampModernAdditionalMount] = []) throws -> WinampModernLoadedSkin {
        ...
        // — the skin's own gate is that param. After the configuration exists, because the claim is
        // the user's per-skin choice to decline.
        WinampModernAvailableComponents.claim(in: runtime.graph) { kind in
            switch kind {
            case .waveformSeeker:
