...
enum LocalRadioHistoryInterval: String, CaseIterable {
    case off = "off"
...
// MARK: - LocalRadioHistory

class LocalRadioHistory {
    static let shared = LocalRadioHistory()

    private var db: Connection?

    // Table
    private let table = Table("local_radio_history")

    // Columns
    private let colId = Expression<Int64>("id")
    private let colTrackUrl = Expression<String>("track_url")
    ...
    private func setupDatabase() {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            NSLog("LocalRadioHistory: Cannot locate Application Support directory")
            return
        }
        let dir = appSupport.appendingPathComponent("NullPlayer")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let dbPath = dir.appendingPathComponent("local_radio_history.db").path

        do {
            let connection = try Connection(dbPath)
            db = connection
            try createTableIfNeeded(connection)
            NSLog("LocalRadioHistory: Database ready at %@", dbPath)
        } catch {
            NSLog("LocalRadioHistory: Failed to open database: %@", error.localizedDescription)
        }
    }
    ...
    private func createTableIfNeeded(_ connection: Connection) throws {
        try connection.run(table.create(ifNotExists: true) { t in
            ...
            t.column(colTrackUrl)
            t.column(colTitle)
            t.column(colArtist)
            t.column(colAlbum)
            t.column(colPlayedAt)
            t.column(colNormalizedKey)
            t.unique(colTrackUrl)
        })
        try connection.run(
            "CREATE INDEX IF NOT EXISTS idx_local_radio_history_played_at ON local_radio_history (played_at)"
        )
        try connection.run(
            "CREATE INDEX IF NOT EXISTS idx_local_radio_history_normalized_key ON local_radio_history (normalized_key)"
        )
    }
    ...
    func recordTrackPlayed(_ track: Track) {
        ...
        let nKey = LocalRadioHistory.normalizedKey(title: track.title, artist: track.artist)
        ...
        do {
            ...
            NSLog("LocalRadioHistory: Failed to record track: %@", error.localizedDescription)
        }
    }
    ...
    func filterOutHistoryTracks(_ tracks: [Track]) -> [Track] {
        ...
        do {
            let rows = try db.prepare(
                table.select(colTrackUrl, colNormalizedKey)
                     .filter(colPlayedAt >= cutoff)
            )
            var trackUrls = Set<String>()
            var normalizedKeys = Set<String>()
            for row in rows {
                trackUrls.insert(row[colTrackUrl])
            ...
            NSLog("LocalRadioHistory: Failed to query history for filtering: %@", error.localizedDescription)
            return tracks
        }
    }
    ...
    func fetchHistory() -> [LocalRadioHistoryEntry] {
        guard let db = db else { return [] }
        do {
            let rows = try db.prepare(table.order(colPlayedAt.desc))
            return rows.map { row in
                LocalRadioHistoryEntry(
                    id: row[colId],
                    trackUrl: row[colTrackUrl],
                    title: row[colTitle],
            ...
            NSLog("LocalRadioHistory: Failed to fetch history: %@", error.localizedDescription)
            return []
        }
    }
    ...
    func removeEntry(id: Int64) {
        ...
        do {
            ...
            NSLog("LocalRadioHistory: Failed to remove entry %lld: %@", id, error.localizedDescription)
        }
    }
    ...
    func clearHistory() {
        ...
        do {
            ...
            NSLog("LocalRadioHistory: Failed to clear history: %@", error.localizedDescription)
        }
    }
    ...
    func generateHistoryHTML() -> String {
        ...
        <title>Local Radio History</title>
        <style>
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .track-id { color: #888; font-size: 0.82rem; font-variant-numeric: tabular-nums; word-break: break-all; }
        </style>
        </head>
        <body>
        <h1>Local Radio History</h1>
        ...
        function removeEntry(id) {
          fetch('/local-radio-history/delete/' + id, {method: 'POST'})
