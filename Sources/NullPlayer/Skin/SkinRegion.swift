...
class RegionManager {
    ...
    /// Note: Y coordinates are in skin's coordinate system (top-down)
    ...
    func hitTest(point: NSPoint, in windowType: WindowType, windowSize: NSSize) -> PlayerAction? {
        ...
        // Convert macOS coordinates (origin bottom-left) to skin coordinates (origin top-left)
        let skinPoint = NSPoint(x: point.x, y: windowSize.height - point.y)
        
        for region in regions {
            if region.rect.contains(skinPoint) {
                // For sliders, calculate the actual value based on position
                return adjustedAction(for: region, at: skinPoint)
            }
        }
