...
enum EmbyRadioHistoryInterval: String, CaseIterable {
    case off = "off"
...
// MARK: - EmbyRadioHistory

class EmbyRadioHistory {
    static let shared = EmbyRadioHistory()

    private var db: Connection?

    // Table
    private let table = Table("emby_radio_history")
    ...
    private func setupDatabase() {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            NSLog("EmbyRadioHistory: Cannot locate Application Support directory")
            return
        }
        let dir = appSupport.appendingPathComponent("NullPlayer")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let dbPath = dir.appendingPathComponent("emby_radio_history.db").path
    ...
    private func createTableIfNeeded(_ connection: Connection) throws {
        ...
        try connection.run(
            "CREATE INDEX IF NOT EXISTS idx_emby_radio_history_played_at ON emby_radio_history (played_at)"
        )
        try connection.run(
            "CREATE INDEX IF NOT EXISTS idx_emby_radio_history_normalized_key ON emby_radio_history (normalized_key)"
        )
    }
    ...
    func recordTrackPlayed(_ track: Track) {
        guard let db = db,
              let trackId = track.embyId else { return }

        let serverId = track.embyServerId ?? EmbyManager.shared.currentServer?.id ?? ""
        let nKey = EmbyRadioHistory.normalizedKey(title: track.title, artist: track.artist)
    ...
    func filterOutHistoryTracks(_ tracks: [Track]) -> [Track] {
        ...
        do {
            ...
            return tracks.filter { track in
                if let tid = track.embyId, trackIds.contains(tid) {
                    NSLog("EmbyRadioHistory: Filtered out '%@' by '%@' (track id match: %@)",
                          track.title ?? "", track.artist ?? "", tid)
                    return false
                }
                let nk = EmbyRadioHistory.normalizedKey(title: track.title, artist: track.artist)
                if normalizedKeys.contains(nk) {
                    NSLog("EmbyRadioHistory: Filtered out '%@' by '%@' (normalized key match: %@)",
    ...
    var historyPageURL: URL? { URL(string: "http://127.0.0.1:8765/emby-radio-history") }
    ...
    func generateHistoryHTML() -> String {
        ...
        <title>Emby Radio History</title>
        <style>
        ...
        function removeEntry(id) {
          fetch('/emby-radio-history/delete/' + id, {method: 'POST'})
