...
| ID | Item | Reach | Notes |
|---|---|---|---|
| W38 | `alphaBlendTo` on an element | **26 skins, 40 uses** (`vidBack` 15, `timeColon` 4, `visMask` 4, `mainBack2` 2, …), measured 2026-09-08 over the 179 archives after W37 closed — it is now the **largest row on this page** | The third of WMP's element animation methods beside `moveTo`/`resizeTo`, which are implemented as immediate endpoints; the same treatment applies — set the alpha now, track the tween as rendering work. It is already in `elementMethodVocabulary`, so it reports as `UNRECOGNISED` demand rather than dying as a bare `TypeError`. Same for `setColumnWidth` (2 skins). **This is the other half of W68's Alienware/ALX family, and W75 closing is what makes it the next thing to try there**: artwork now swaps from script, so what is left holding those `m_anim_*` GIFs invisible is the subviews authored `alphaBlend="0"` and faded in by a handler this method never reaches. |
