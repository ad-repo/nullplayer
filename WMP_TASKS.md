...
**Start here: W124, in Tier 1e.** **W102 closed 2026-09-10** — `<VIDEO>` is hosted, the picture is
...
behind is ranked here: **W124** (the skin is never told the video came back, so it stays in its
`onVideoEnd` state) and **W125** (no right-click anywhere on a `.wmz` reaches `menu(for:)`, which
takes the subtitle and audio-track menus with it — and the visualization menu that predates them).
W103 is now answerable: the video path exists, so `<VIDEOSETTINGS>`'s four sliders are a decision
about what to bind, not a question about whether there is anything to bind to.
...
`xsn_sports`'s drifting drawer button is now **W99**, `BlueCrush`'s dead title-bar button is
**W100**, and `Ginger Man`'s missing track display is its `mainView` being starved (10 nodes, 2
...
| W99 | A drawer's own toggle button drifts out from under the pointer once the view's timer runs | `xsn_sports` confirmed both live and headlessly; **every skin with a timer and a moved drawer is a candidate — count it** | Reported live 2026-09-09 as "it opens and closes right away, it is resistant to opening", and **reproduced against a baseline worktree at the parent commit, where it behaves identically** — so it is not W55, which only made it visible by correctly hiding a shut drawer's contents. `vidDrawerButton` in `xsn_sports/videoView` is drawn at `61,291` on the first frame and at `61,271` after `WMP_RENDER_SETTLE=2` runs the view's own 500 ms timer; a click at the first position returns `CLICK … MISS`, and a click at the settled one hits and opens the drawer to `4 widgets[slider×4]`. So the drawer works and the *target moves*. Twenty pixels, on a rebuild driven by a timer that only calls `htcpVid()` — which alpha-blends artwork and moves nothing — so find what re-resolves that subtree's geometry between ticks before assuming the handler did it. Likely family: W77, a scripted `view.height` that reaches no window, since `toggleVidDrawer()` positions the drawer at `view.height-131`. Reproduce with `WMP_SKIN=…/xsn_sports.wmz WMP_RENDER_SETTLE=2 WMP_RENDER_PROBE=videoView`. |
| W100 | `view.returnToMediaCenter()` is not implemented, and it is a button a skin puts in its title bar | `BlueCrush_MP7` and `BlueCrush_MPXP` confirmed; **unmeasured corpus-wide** | Reported live 2026-09-09 as "the 2 top right windows do not work". One of the two is `<BUTTON onClick="view.returnToMediaCenter()">`, and the name exists nowhere in `Sources/NullPlayer/WMPSkin/`, so it is an unrecognised member that kills its handler on the first statement. Pre-existing and unrelated to the change that surfaced it. The same skins also demand `theme.openDialog` (×2), `player.launchURL` and `player.url`. **Decide what it should mean before implementing**: WMP returns the user to the Media Center shell, which this player has no equivalent of, so the honest options are `inert()` — the trap `INERT` exists for — or mapping it to the closest thing NullPlayer has. Do not alias it to `closeView` silently. Count the corpus first: sweep for `returntomediacenter` in `UNKNOWN member` lines. |
...
| ID | Item | Reach | Notes |
|---|---|---|---|
| W124 | A skin is told the video ended and never told it came back, so it stays in its `onVideoEnd` state | `onVideoStart` **105 uses / 91 skins**, `onVideoEnd` **77 / 70**, both measured 2026-09-10 over the 180 archives; every skin that pairs them is a candidate and the pairing is unmeasured | **Opened 2026-09-10 by W102 closing.** `WMPVideoPresentation.events` raises `videostart`/`videoend` off `WMPVideoSnapshot.hasVideo`, and `hasVideo` is derived live from `mediaPlayer.hasVideoOut` + `videoSize`, both of which VLC drops while it tears a vout down and rebuilds it. Confirmed live on `Cablemusic`, which authors `onVideoStart="StartVideo();SwitchBig();" onVideoEnd="EndVideo();"`: the trace shows exactly one `INPUT present videoend,…` and **no `videostart` afterwards**, so the skin ran `EndVideo()`, re-enabled its effects rect over the video box, and never got `StartVideo()` back. The picture re-hosted (`INPUT video hosted … source=1920.0x1080.0`) while the *skin* stayed in its ended state — so the two halves disagree about whether a film is playing. **Do not "fix" this by suppressing `videoend`**: a real end of media has to raise it. The shape is almost certainly a latch — hold the last good `WMPVideoSnapshot` for as long as the same media is open in `WMPAudioEngineHost`, and clear it when the media changes or `didReachEndOfMedia` — but the transient was inferred from the symptom and **has not been measured directly**; instrument `hasVideoOut`/`videoSize` across a seek and a restart before writing the latch. Reproduce: play any film into a skin with a video box, then re-trigger the same film, and `grep "present video" ` the `WMP_TRACE_INPUT=1` log. |
| W125 | No right-click anywhere on a `.wmz` reaches `menu(for:)`, so every host menu the skin view offers is unreachable | The whole corpus — `WMPMainView.menu(for:)` is the only host-menu entry point in the subsystem; it currently serves the `<EFFECTS>` rect (**51 skins wire an `onClick` on it**) and, since 2026-09-10, the `<VIDEO>` rect | **Opened 2026-09-10, and it predates the video work.** `WMPMainView.menu(for:)` traces `INPUT menu at=…` on entry; across a full live session with right-clicks over the video box, over plain chrome, and over the effects rect, that line **never appeared once**, while `INPUT hover` updated at the same coordinates on the same tick — so pointer events reach the view and the right-click specifically does not become a menu request. Nothing in `WMPSkin/` overrides `rightMouseDown` or `sendEvent`, and the synthetic right-click was verified to work against other apps, so the cause is not the instrument. **The consequence is bigger than the video row that found it**: the visualization menu this method was written for has the same entry point, so it may never have worked either — check that before assuming this is new. It also strands the only route to subtitle and audio-track selection while the picture is parked in a skin (the command bar is deliberately off there, and the parked window takes no mouse events by design so the skin's own `<VIDEO>` `onClick` — 21 uses — keeps working). Start by proving whether `menu(for:)` is called at all on a borderless `.wmz` window, then whether the child video window's `ignoresMouseEvents` passthrough carries right-clicks the way it carries motion. |
...
| W105 | `WMPSkinSurface` models two of the six surfaces the corpus declares | visualization **171 skins**, video **170**, video settings **94**, network **4** — against playlist 170 and equaliser 163, all measured 2026-09-09 over 179 archives | `WMPSkinSurface` has exactly two cases, and its own doc comment claims "every other window NullPlayer opens … has no counterpart in the WMP skin format at all, so nothing there can be redundant" — **false, and corrected in place 2026-09-09**; four surfaces have a counterpart. The consequence is that the Windows-menu toggles for the visualizer and the video window open ours over a skin that declares its own, which is the exact defect W93 was opened to stop for the playlist. The change is small and its ordering is not: adding a case makes `routeWMPSkinSurface` stand our window aside, so **a case added before its surface is hosted trades a duplicate window for an empty drawer** — this tier's founding rule. W101 has landed, so `.visualization` may go in now; **`.video` landed 2026-09-10 with W102** — `routeWMPSkinSurface(.video, switchingViews: true)` now backs `toggleVideoPlayer`, `isVideoPlayerVisible` answers from the skin when the skin owns the box, and the Windows-menu item is built by `buildSkinOwnableWindowItem` like Playlist and Equalizer; `.networkStats` may go in ahead of them because Flow's own window is the fallback either way, and `.videoSettings` earns a case only if W103 implements something. The three shapes the routing has to answer are unchanged and are in `skills/wmp-skin-guide/SKILL.md` § *Ask what the skin provides before opening a window of your own*. |
| W106 | The `<EFFECTS>` rect's own attributes are parsed and unhonoured | `windowed` **125 skins** (`"false"` in 108), `clippingColor` 44, `allowAll` 11, `clippingImage` 12, `effectCanGoFullScreen` 12, measured 2026-09-09 with `scripts/wmp_markup_census.sh <outdir> EFFECTS WMPEFFECTS` | Left standing by W101, which hosted the surface and answered its four live members. `windowed="true"` (17 uses) says WMP drew the visualization in a window of its own rather than inline, and this engine draws it inline either way — which is right for a box the skin sized, and wrong for a skin that expected a separate window; **look at one of the 17 before implementing anything**. `clippingColor`/`clippingImage` are the same colour-key class the renderer already implements for artwork, applied to a live surface, and they are the reason W74's `WMPWidget.clipRect` matters here too. `visEffects.settings()` (9 skins) belongs with them: it opens WMP's own visualizer property sheet, and the honest options are `inert()` or the surface's own context menu, which W101 already builds. |
