...
`UNRECOGNISED` where it belongs. **W136 and W131–W133/W135 are that audit's remaining rows, ranked
in that order** (W136 in § 2a; W131–W133 and W135 in Tier 3), and § *2c-note* is its
...
| W131 | `hoverForegroundColor` and the TEXT hover-colour family are never read | `hoverForegroundColor` **212 uses / 40 skins**, `hoverBackgroundColor` 5 / 1, `disabledFontStyle` 20 / 2 | No occurrence of either hover colour anywhere under `Sources/NullPlayer/WMPSkin/` or `Sources/NullPlayer/Windows/WMPSkin/`. The hover *artwork* path exists (`WMPSceneBuilder.swift:490-493`) and TEXT nodes have been hit targets since Phase 7, so the state is available and only the colour roles are missing: a link row meant to light up under the pointer is inert. **W130 supplies the named-colour parser this row needs** — 34 of `hoverForegroundColor`'s values are the named `blue`; the state-specific renderer work remains open. **Needs the live loop**: a default-state capture never enters a hover (W73), so `WMP_RENDER_HOVER` plus a debug build with `WMP_TRACE_INPUT=1`. |
