...
| W56 | Video and playback-position events | `onvideostart` 190/140, `onvideoend` 132/130, `onpositionchange` 147/41, `currentposition_onchange` 103/80 | `onvideostart`/`onvideoend` reach 140 and 130 skins because the standard WMP template wires them; both need the hosted video surface (Phase 5, W9) before they can be honest. **`currentposition_onchange` is one instance of W129, not a mechanism of its own** (amended 2026-09-11 by the SDK audit): the SDK defines `<attribute>_onchange` for every attribute of most elements, this engine collects it for four geometry properties plus `value`, and the position spelling is simply the highest-reach name in that general class. Take it there. The `onvideostart`/`onvideoend` half stays here and stays blocked on the video surface. |
| W129 | `<attribute>_onchange` is a five-property whitelist against a general SDK mechanism | **387 authored handlers across 104 of the 177 measured archives**, 20 distinct properties: `currentPosition_onchange` 77 skins, `currentEffectType_onchange` 57, `currentPlaylist_onchange` 48, `currentPreset_onchange` 30, `textWidth_onchange` 17, `selectedItem_onchange` 12, `playlist_onchange` 9, `currentMedia_onchange` 9 | Largest reach in the SDK audit. *Ambient Event Handlers*: "when a skin attribute changes value, an event occurs… the name of the event handler is the name of the attribute followed by `_onchange`". **This is three pieces, not two**, and the audit's own description of it was wrong in a way that matters: it is only true that a non-geometry `_onchange` "is parsed as a handler and never dispatched" for names in `WMPAttributeParser.handlerNames` (`WMPAttributeValue.swift:50-82`), which is a **closed list**. `currentPosition_onchange`, `currentEffectType_onchange`, `currentPlaylist_onchange` and the rest are not in it, so they are classified `.literal`/`.jScript` and **are not handlers at all**. So: (1) classification — `handlerNames`; (2) collection — `WMPScriptViewPlan`, today gated on `WMPScriptExpression.geometryProperties` (`WMPScriptContext.swift:83-93`, `:131`); (3) dispatch. **Breadth is decided**: raise from element mutations *and* from the host snapshot diff in `WMPMainWindowController` (`currentPosition`, `currentMedia`, `currentPlaylist`, `currentEffectType`), because the host-driven half is where the measured reach is. Raise it from the same bounded once-per-`(element, property)`-per-transaction cascade `raiseGeometryChanges` already uses, and **the W87 counter-evidence binds: raise only what the markup declared, never re-resolve the expression set**. The failure mode is the silent one — a readout that never repaints with no error anywhere, because nothing threw; `currentEffectType_onchange` is the sharpest case, since W101 made the effect selection live and writable and a skin painting its effect-name label from that handler still shows the label it loaded with. **Needs the live loop** (`reference/harness.md` § *Driving the app*, with playback): a headless still never triggers a change. |
...
| W130 | Two colour parsers and only one knows names, so ~92 skins' declared chrome is invisible to the palette | `backgroundColor` authored as a name in **92 skins** (`black` 124 uses, `pink` 14, `blue` 3, `white` 1); `foregroundColor` in **22 skins** (`white` 52, `black` 37). Measured 2026-09-11 over the 177 archives | *Color Reference*: "every attribute that takes a color value can be specified using one of the following color names or hexadecimal values" — **140 names**. `WMPAttributeParser.color(from:)` (`WMPAttributeValue.swift:138-144`) is a strict 7-character `#RRGGBB` parse; `WMPSceneBuilder.color(_:names:)` (`WMPSceneBuilder.swift:829-844`) falls back to a private **five**-name table (`black`, `white`, `red`, `green`, `blue`). `WMPSurfacePalette.swift:103` uses the strict one, so a skin declaring `backgroundColor="Black"` is counted as declaring nothing and falls through to dominant-colour sampling or the neutral fallback for NullPlayer's own hosted windows. The separate visible symptom is `backgroundColor="pink"` (#FFC0CB, **14 uses / 8 skins**): unresolved in the scene builder too, so the `.fill` command is never emitted and the element draws no background at all. **`green` is worth noting as already correct** — the engine's `#008000` matches the SDK chart, not the `#00FF00` one might assume. Fix: one shared named-colour table behind `WMPAttributeParser.color(from:)`, with the scene builder's private switch deleted in favour of it. **`"none"` must stay unparsed** — the SDK defines it as *absence*, and the corpus authors it 554 times across 143 skins. **This row impeaches a number in `SKILL.md`**: § *NullPlayer's own windows beside a skin* says "only 96 archives declare any background colour and 95 any foreground", measured with the strict parser; re-measure the declaration counts and correct that section **in the same change**. Sweep the corpus — this one touches the scene. |
| W131 | `hoverForegroundColor` and the TEXT hover-colour family are never read | `hoverForegroundColor` **212 uses / 40 skins**, `hoverBackgroundColor` 5 / 1, `disabledFontStyle` 20 / 2 | No occurrence of either hover colour anywhere under `Sources/NullPlayer/WMPSkin/` or `Sources/NullPlayer/Windows/WMPSkin/`. The hover *artwork* path exists (`WMPSceneBuilder.swift:490-493`) and TEXT nodes have been hit targets since Phase 7, so the state is available and only the colour roles are missing: a link row meant to light up under the pointer is inert. **Lands with W130** — 34 of `hoverForegroundColor`'s values are the named `blue`, so on its own this row would light up 178 of 212 uses and leave a family of skins unexplained. **Needs the live loop**: a default-state capture never enters a hover (W73), so `WMP_RENDER_HOVER` plus a debug build with `WMP_TRACE_INPUT=1`. |
