...
## Milkdrop Visualization Window

### Overview
The Milkdrop visualization window displays real-time audio visualizations using OpenGL. It supports spectrum analyzer and oscilloscope modes, with the architecture designed for future projectM (Milkdrop) preset support.

### Window Structure
- **Title Bar**: 14px height, uses custom milkdrop_titlebar.png sprite sheet
- **Side Borders**: 6px on each side, using playlist side tile sprites
- **Bottom Border**: 6px height, using playlist bottom tile sprites
- **Visualization Area**: OpenGL view fills the content area inside the borders

### Sprite Sources

| Element | Source | Sprite |
|---------|--------|--------|
| Title Bar (Active) | PLEDIT.BMP | Playlist TitleBarActive sprites (left corner, tile, right corner) |
| Title Bar (Inactive) | PLEDIT.BMP | Playlist TitleBarInactive sprites |
| Shade Background | PLEDIT.BMP | PlaylistShade sprites (left corner, tile, right corner) |
| Close Button | TITLEBAR.BMP | Standard close button sprites |
| Shade/Unshade Button | TITLEBAR.BMP | Standard shade/unshade sprites |

### Layout Constants (SkinElements.Milkdrop)
```swift
minSize = NSSize(width: 275, height: 150)
defaultSize = NSSize(width: 400, height: 300)
titleBarHeight = 14     // Matches scaled PNG content height
shadeHeight = 14
...
TitleBarButtons:
  ...
  shadeOffset = 20    // From right edge
```

### Title Text
- Uses pixel font (5x6 pixels per character) for "MILKDROP" text
- Gold/yellow color when active: RGB(0.85, 0.75, 0.35)
- Muted gray when inactive
- Same style as other playlist/library title bars

### Shade Mode
In shade mode, the window collapses to a 14px height title bar:
- Uses Playlist shade background sprites (same as playlist window shade)
- Title "MILKDROP" displayed using pixel font (gold/yellow)
- Close and unshade buttons positioned relative to right edge
- OpenGL visualization is hidden and CVDisplayLink stopped

### Visualization Modes

1. **Milkdrop (projectM)** (default when available)
   - Full Milkdrop preset (.milk file) rendering via libprojectM v4.x
   - Real-time beat detection and audio-reactive visuals
   - Preset navigation: Next/Previous/Random with smooth or hard cuts
   - Preset auto-switching with configurable duration
   - PCM audio data from AudioEngine.pcmData
   - Requires libprojectM-4.dylib in Frameworks/

2. **Spectrum Analyzer** (fallback)
   - 75 bars representing frequency bands
   - Colors from green (low) to red (high) frequency
   - Data from AudioEngine.spectrumData

3. **Oscilloscope** (fallback)
   - Waveform display of raw PCM audio
   - Green line on black background
   - Data from AudioEngine.pcmData

### Audio Data Integration

**Spectrum Data (75 bands)**:
- Generated by AudioEngine.processAudioBuffer()
- FFT analysis with Hann window
- Logarithmic frequency mapping (20Hz-20kHz)
- Fast attack, slow decay smoothing

**PCM Data (1024 samples)**:
- Raw mono audio samples (-1.0 to 1.0)
- Downsampled from FFT buffer
- Updated at 60fps via display timer

### OpenGL Implementation (VisualizationGLView)

- **Context**: OpenGL 3.2 Core Profile (macOS max is 4.1)
- **Rendering**: CVDisplayLink for vsync'd 60fps
- **Shaders**: Simple vertex/fragment for bar and line rendering
- **Buffer**: Single VAO/VBO with dynamic vertex data

### CVDisplayLink Lifecycle
- Started when: Window shown, un-minimized, exits shade mode
- Stopped when: Window hidden, minimized, enters shade mode, closed
- Managed by MilkdropView via viewDidMoveToWindow, viewDidHide, etc.

### ProjectM Integration (Implemented)

The visualization system now supports libprojectM v4.x for real Milkdrop preset visualization:

**Architecture:**
- `ProjectMWrapper.swift` - Swift wrapper around libprojectM C API
- `CProjectM` system library target - Module map for libprojectM headers
- Conditional compilation - Gracefully falls back when library unavailable

**Setup Requirements:**
1. Build libprojectM v4.1.6+ as universal binary (arm64 + x86_64)
2. Place `libprojectM-4.dylib` in `Frameworks/`
3. Uncomment `CProjectM` dependency in Package.swift
4. Add preset files (.milk) to `Resources/Presets/`
5. Add textures to `Resources/Textures/`
...
make -j8
# Copy libprojectM-4.dylib to Frameworks/
```

**Preset Navigation:**
- Right/Left Arrow: Next/Previous preset
- Shift + Arrow: Hard cut (instant switch)
- R: Random preset
- L: Lock/unlock current preset
- Context menu for all preset controls

**PCM Data Flow:**
- Local files: AudioEngine tap → pcmData → VisualizationGLView → ProjectMWrapper
- Streaming: StreamingAudioPlayer → delegate → AudioEngine.pcmData → visualization

### Implementation Files
- `Sources/AdAmp/Visualization/ProjectMWrapper.swift` - libprojectM Swift wrapper
- `Sources/AdAmp/Windows/Milkdrop/MilkdropWindowController.swift`
- `Sources/AdAmp/Windows/Milkdrop/MilkdropView.swift`
- `Sources/AdAmp/Windows/Milkdrop/VisualizationGLView.swift`
- `Sources/AdAmp/Skin/SkinElements.swift` (Milkdrop struct)
- `Sources/AdAmp/Skin/SkinRenderer.swift` (drawMilkdropWindow, etc.)
- `Frameworks/libprojectm-4/projectM-4.h` - C API header
- `Frameworks/libprojectm-4/module.modulemap` - Swift module map
