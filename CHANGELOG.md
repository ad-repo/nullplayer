...
- **CLI server queries no longer return empty results at launch** — headless `--cli` queries and playback against Plex, Jellyfin, and Emby (`--list-artists`, `--list-albums`, `--list-tracks`, `--artist`, `--search`, etc.) now wait for the background server connect/refresh before running, instead of racing it and silently returning nothing (which surfaced as "artist not found" / "0 artist(s)"). When the restored current library is a non-music section — a Plex Movies/TV library, or a Jellyfin/Emby "Playlists", "Video", "Movies", or "TV shows" view — the CLI now auto-selects a music library for music queries, or prints the available music libraries and asks you to pick one with `--library` instead of returning empty. `--list-sources` also waits for those connect tasks, so configured Subsonic/Navidrome, Jellyfin, and Emby servers show as **Connected** instead of momentarily "Not configured".
