...
- **Cava is now available in the main-window visualization display** — the classic and modern/metal 76×16 analyzer area can render a mono Cava spectrum that follows the active skin palette by default. Its color preset, bar count, smoothing, and bass tilt are independently configurable under **Visuals > Main Window**, without changing the standalone Cava window. The **Visuals** menu is now organized into **Main Window**, **Spectrum Window**, and **Visualizations** submenus; the visualization-engine choice is labeled simply **ProjectM**.
