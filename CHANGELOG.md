...
- **YouTube → Sonos (experimental paste-URL middleman)** — a new **Output → Streaming → Sonos (Experimental) → Open Video URL → Sonos…** action plays a YouTube (or any yt-dlp-supported) video **locally and muted** in NullPlayer's video window while streaming its **audio to Sonos**, keeping the watchable video on the Mac. NullPlayer resolves the URL with `yt-dlp`, picks a ≤1080p h264 video-only stream plus the highest-bitrate audio-only stream, proxies AAC audio through the embedded HTTP server with the original request headers when available, and falls back to a producer-backed live ADTS/AAC `ffmpeg` transcode only when the source audio is not Sonos-friendly. The local video now opens muted but paused, waits for a direct Sonos `PLAYING` confirmation, then starts from the confirmed Sonos position instead of racing ahead and trying to repair sync afterward. An **A/V offset slider** opens at `0` by default for each video in the video control bar; releasing it applies one deliberate video time shift for manual calibration. **Stop YouTube → Sonos** tears the whole pipeline down cleanly (unregisters the proxy or live stream, kills ffmpeg when used, stops the cast). The feature is gated on the presence of the `yt-dlp` and `ffmpeg` binaries and is hidden when they're absent — install them to enable it (e.g. `brew install yt-dlp ffmpeg`), point at custom paths via `NULLPLAYER_YTDLP_PATH`/`NULLPLAYER_FFMPEG_PATH`, or use a DMG build with the binaries bundled. The sandboxed Mac App Store build cannot run external binaries, so the feature stays hidden there by design. *v1 limitations:* sync is coarse (not frame-accurate), and whole-timeline seeking still has to seek both Sonos and the video. See the `sonos-casting` skill for details.
