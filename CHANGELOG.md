...
- **YouTube downloads track each format separately** — the download manifest is now keyed by `(videoId, format)` instead of just the video ID, so a video already grabbed in one format (e.g. FLAC) no longer reports as "downloaded" for every other format. With the new video formats this matters: previously, downloading the audio first made both browsers show **Play** and replay the audio, with no way to also fetch the 720p/1080p video (or a different audio format) short of removing the existing download. Now **Play** / **Download & Play**, the downloaded marker, and **Remove Download** all reflect the format currently selected in Library → YouTube → Format, and multiple formats of the same video can coexist. Older manifests are migrated automatically (format inferred from the file extension).
