...
- **Windows Media Player `.wmz` skins** — a Windows Media Player mode that opens in a WMP-owned unskinned player with normal transport controls and an in-app Import button. The UI menu can import, select, remove, and switch views in user-supplied `.wmz` skins, save compatibility reports, or return to the unskinned player. Existing Classic, Original, Original-Metal, and WMP mode choices are preserved on upgrade. Skin archives run behind bounded resource checks sized for how WMP skins are actually authored — including the very wide filmstrip artwork skins use for sliders and progress bars — and a skin's own JScript now runs for real: each skin gets one sandboxed script session that holds its state, so a skin's setup code, its computed layout, and its own buttons work the way they do in Windows Media Player — a panel that opens on one click closes again on the next. Skins draw their own controls, so a `.wmz` looks and behaves like itself rather than like a macOS approximation: sliders show the skin's own thumb artwork and move along the axis the skin drew them on — vertical equalizer bands included — seek and volume bars fill with the skin's own progress art, and the skin's ten equalizer sliders drive the real equalizer instead of a grey panel drawn on top of them. Text is drawn in the face, size and style the skin asks for, elements the skin means to be invisible no longer paint over its artwork, and a decorative overlay no longer swallows clicks meant for the buttons underneath it. Skins with a shaped window — round players, cut-out artwork — are shaped again instead of sitting inside a black or grey box, animated artwork animates, custom-drawn dial and arc sliders follow the path the skin drew for them, and the pointer changes shape over a control when the skin says it should. The black rectangle that used to cover the artwork of skins with a video pane is gone. A skin's own handlers now run for the events it actually writes: nearly every WMP skin wires its sliders, its play/open state and its readouts using WMP's `value_onchange`, `PlayState_onchange` and `OpenState_onchange` spellings, and those are now recognised — so a skin's equalizer bands, volume readouts and state-driven artwork respond where they previously sat silent. A skin can also fade its own elements: the `alphaBlendTo` call WMP skins use to bring artwork in and out now works, so setup code that used to stop at that line runs to the end — skins that pick a colour scheme for themselves choose it, drawers and panels open and close in the state the skin asks for, and dimmed labels are dimmed rather than fully lit. Magenta no longer shows through a skin's artwork: Windows Media Player treats magenta as the see-through colour for any artwork whose author did not name one, and NullPlayer now does the same — including on the hover and pressed states of a button, which previously kept their magenta and flashed a bright pink block under the pointer. Animations also play the way the skin drew them instead of restarting: an intro that opens once now opens once and stays put, rather than snapping back to its first frame every time the skin repainted itself — which, on a skin with a fast timer, was many times a second, and on every one of them as soon as you moved the mouse over the window. A skin's own animation can now finish: an intro that ends by revealing the player and switching its own timer off does that and stays there, instead of reaching the player and then immediately playing itself back into its closed state — after which nothing on it answered. ActiveX, registry, shell, DLL, plug-in, filesystem, and network access remain unavailable; a skin asking for something NullPlayer does not implement loses only that one action, not the skin.
