...
- **Compact Mode — menu-bar mini player (classic & modern UI)** — a new **Compact Mode** (main window right-click → **Compact Mode**, or the `Windows` top menu) collapses NullPlayer into a single menu-bar app: the Dock icon is hidden, a NullPlayer status-bar item appears, and the only window is the **Library Browser** with a stripped-down player bar embedded across the top — transport, seek + time, a scrolling title marquee, and volume — built from the active UI's own components (authentic classic skin sprites in classic, modern controls in modern). Clicking the status item reveals the window; its menu also offers **Exit Compact Mode**. All other windows are hidden on entry and their prior visibility is restored on exit, along with the Dock icon and the macOS menu bar. The one exception is the **video player window**: a playing video (e.g. a downloaded YouTube video) stays visible when entering Compact Mode, and videos opened while compact appear normally.

- **Audio Analysis window — Friture-style multi-pane analyzer** — a new classic and modern UI **Audio Analysis** window (Window menu / right-click → **Audio Analysis**) gives a real-time, multi-pane view of what's playing, modeled on [Friture](https://friture.org). Its right-click menu switches between three panes: a **Scope** (oscilloscope) drawing the live mono waveform; a **Levels** meter showing per-channel peak and RMS in dBFS with color-coded headroom; and a **Spectrogram** — a scrolling Metal waterfall that maps each frame's FFT magnitudes through a perceptually-uniform Viridis color ramp (low frequencies at the bottom, time scrolling right-to-left). The window docks and snaps with Main/EQ/Playlist/Spectrum, restores its frame with **Remember State on Quit**, and remembers the selected pane. Each pane registers as an audio consumer only while it's the visible pane and the window is open, so a hidden or closed window leaves the FFT and audio taps idle (the spectrogram also pauses rendering when the window is minimized). The peak/RMS, octave-band, pitch (autocorrelation), and L/R delay (cross-correlation) routines live in a new, unit-tested pure-Swift DSP module shared for future panes.
...
- **Metal mode — hi-fi faceplate finishes** — Metal mode is a fully realized hardware look, selectable from **Skins → Metal**, with seven built-in finishes: **Brushed Steel** (default), **Aluminum** (bright polished silver), **Gunmetal** (dark blue-gray satin), **Anodized Black** (near-matte charcoal), **Brass** (warm polished yellow-gold), **Bronze** (dark muted brown-gold), and **Copper** (warm reddish-orange). Each finish is a per-finish `MetalMaterial` that drives the whole chrome — window sheen, panels, sliders, transport icons, and EQ (with a brightness-based fader ramp) — with on-chrome text automatically light on dark finishes and dark on light ones. The time/track displays and EQ curve render on a backlit-green LCD with dark "ink", and the spectrum analyzer uses the **vis_classic** core running a per-finish profile drawn transparent over the chrome, with a bar gradient matched to each finish (re-applied on every load so it always follows the finish). **Skins → Metal/Modern → Reset Skin to Default** restores a skin's shipped defaults, discarding persisted visualization overrides.
...
- **Modern main-window toolbar updated** — the visualizer toggle is now labeled **VZ**, and the former **SK** skin-menu button is now an **AA** toggle for opening and closing the Audio Analyzer. The toolbar order is now **SH RP CP VZ EQ PL SP AA WV LB**; modern skins remain available from the menu bar.

- **CP (Compact Mode) toggle button (modern UI)** — the modern main window's third toolbar button is now **CP**, a boxed toggle that enters Compact Mode (replacing the former **CA** cast button — casting stays available from the right-click → Output Devices menu). Clicking it forces the button's active highlight and paints it to screen before performing the switch, so the click registers immediately instead of looking like a hang while Compact Mode tears down the regular windows.

- **Visualization window renamed "Visualizations"** — the window that hosts ProjectM/Geiss/Tripex/Met Museum (formerly shown as "ProjectM" / "projectM") is now titled **Visualizations** everywhere it's user-facing: the `Windows` menu, the classic and modern window title bars, and the modern-skin drawn title.

- **Compact Mode set apart in the Windows menus** — in both the main-window right-click menu and the `Windows` top menu, **Compact Mode** is now on its own line, separated above **Always On Top**, instead of being grouped in with the other display toggles.
...
- **Metal mode — Library Browser top bar no longer darkens intermittently** — in Metal mode the Library Browser's top-chrome (server bar, tab bar, column-header band, alphabet index) is meant to blend into the single brushed-metal surface, but it would sometimes paint the *modern* darkened band over the metal chrome and then clear again on the next redraw. The cause was that "is this metal?" was read from the engine's global render-style flag, which is separate from the skin actually being drawn and could briefly disagree across a skin swap. Metal-ness now travels with the skin instance (`ModernSkin.renderStyle`, stamped at load), and the renderer, the Library Browser, and the spectrum colors all derive it from the skin they're rendering — so the background and its bands can no longer disagree. The Library's scroll fast-path also rebuilds its renderer from the current skin so the background can't lag a swap.
- **Plex Library search artist navigation now lands on the selected artist** — selecting an Artist from Library Browser search results now switches to the Artists tab and selects the matching artist by title after the destination list has loaded and any column sort has been applied. Previously Plex search-result artist IDs could differ from the normal Artists tab rows, and the preserved selection index could then point at a different artist after sorting. Fixed in both modern and classic UI paths.
- **Cleartext `http://` internet-radio streams play again (#310)** — after 0.25.0, `http://` radio stations (Icecast/SHOUTcast) appeared to connect but produced no audio, while `https://` stations worked. A `NSAllowsLocalNetworking` key (added for casting) had been sitting alongside `NSAllowsArbitraryLoads` in the App Transport Security settings; on macOS 10.12+ pairing the global key with *any* granular ATS key makes the system ignore the global one, so public cleartext hosts were silently blocked with CFNetwork error `-1022` (the stream connected, never advanced past `0:00`). Removing the redundant `NSAllowsLocalNetworking` lets `NSAllowsArbitraryLoads` apply again — it already covers local-network casting, so Sonos/Chromecast/DLNA are unaffected.
- **"Test" button in Add Station no longer false-fails working streams (#310)** — the test used a `HEAD` request, but many Icecast/SHOUTcast servers reject `HEAD` with `400`/`405` even though they stream fine over `GET`, so the test reported an error for stations that play perfectly. It now probes the way the audio player actually connects — a `GET` with `Icy-MetaData`, cancelled once the response headers arrive — so the result matches real playback.
...
- **Library Browser ART mode exits when you switch tabs or change source** — clicking a different Library tab (Artists, Albums, etc.) or switching the browse source now leaves ART mode and restores the normal browser list, in both classic and modern UI, so you're no longer stuck looking at artwork after navigating away.
...
- **Casting to a recently rebooted Sonos speaker self-heals** — casting to a Sonos speaker that had just rebooted failed with an HTTP 500 SOAP error, because the cached group-coordinator address went stale (a reboot isn't an IP change, so topology was never re-fetched before the cast). Previously the only fix was to manually refresh the device list and try again. Now a recoverable Sonos cast failure automatically re-fetches the group topology, re-resolves the coordinator for that room, and retries the cast once — so the first cast after a speaker reboot just works. Recovery is Sonos-only and runs at most once; genuinely fatal errors (Connection Security 401/403, unsupported format) are not retried and surface their existing message.
- **Sonos cast recovers when the speaker reboots mid-stream (#304)** — rebooting a Sonos speaker *while* it was casting left NullPlayer desynced from reality: the UI kept showing "casting" but Play came out of the Mac's local speakers, pressing Cast again produced **double playback** (local *and* Sonos at once), and the only way out was a manual Stop then Start. Nothing tore down the cast session when the speaker went unreachable — a reboot isn't an IP change, so the existing recovery never fired, and the dead session lingered indefinitely. Now the playback poller counts consecutive failures and, after 3 in a row (~15s), cleanly tears the session down — the cast indicator clears, Play resumes locally, and re-casting starts a single clean stream. This is teardown-only with no auto-reconnect (distinct from the initial-cast self-heal above, which retries a stale coordinator *before* playback starts).
