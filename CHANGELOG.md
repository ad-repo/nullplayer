...
- **UI Size adds a Medium option** — the old **Large UI** toggle is now a **UI Size** submenu with **Normal (100%)**, **Medium (125%)**, and **Large (150%)** radio choices in both the Windows menu and the main-window context menu. Medium and Large resize live in Classic, Modern, and Metal UI; switching between modes while an enlarged size is active restores the exact selected size, and saved sessions now remember Medium while still restoring older Large UI sessions correctly.
