...
- **Fake lossless detection reporting** — Playback Options now includes an opt-in **Fake Lossless Detection Reporting** toggle for lossless-looking audio (`FLAC`, `ALAC`, `WAV`, `AIFF`, PCM). When enabled, NullPlayer decodes a bounded PCM sample off the playback path and reports probabilistic authenticity evidence in Now Playing: confidence, scan coverage, spectral cutoff, high-band energy, and hi-res upsample warnings. The analyzer uses a Hann-windowed vDSP FFT over independent channels with per-band max combining, ignores near-silence, avoids treating metadata as proof, and caches only settled in-memory results for the current session. The toggle persists in UserDefaults and Remember State, posts its own status notification, and never blocks playback; unsupported/lossy formats and live streams degrade to not-applicable or inconclusive status.
