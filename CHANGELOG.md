...
- **Podcasts are now a first-class Library source** — Classic, Modern, and Metal Library browsers can search the open Podcast Index directory, subscribe to shows, browse cover tiles and episodes, resume playback, favorite or download episodes, and send audio or video episodes through the shared playlist, local player, and casting paths. Podcast library state lives in the shared database, credentials stay in Keychain, and existing RSS/Atom feeds remain the playback fallback.
...
- **Podcast browsing is now centered on search and subscriptions** — the Podcasts page has a single full-width search field instead of a section switcher and toolbar buttons. Submitting a query enters search mode, Back returns to subscriptions, and trending results are no longer fetched automatically. The Library source is consistently named **Podcasts**, with a separator between YouTube and Podcasts in both source menus.

### Bug Fixes

- **Compact Playlist mode now shows the configured album art instead of stale source content** — the Modern/Metal queue draws the current artwork in the same content rectangle used by the Library list and hides opaque History or Podcasts hosts while the queue is active, preventing a podcast background—or a blank layer—from covering the playlist backdrop.
