...
- **Winamp 5.x modern skins (`.wal`)** — a new **Modern** UI mode under the skin menu loads and runs real Winamp 5.x modern skins: the Wasabi XML/XUI markup is parsed into a live object graph and the skin's compiled MAKI scripts are executed by a built-in interpreter, so the skin's own buttons, sliders, text, animations, and alpha-shaped window all work. Import a skin with **Import .wal Skin…**; skins that depend on the separate ClassicPro plugin can point **Import ClassicPro Engine…** at the installer you already have, which is unpacked entirely inside nullPlayer (nothing is downloaded, and no installer is ever run). Playlist and equalizer surfaces that a skin embeds in its own window are hosted in place and driven by the real playback engine. **Known limitations:** most widgets built entirely on one of Winamp's own standard-library shells draw empty (the colour-theme list and text buttons are drawn for you; a scrollbar next to one does nothing), the embedded playlist and EQ are drawn by nullPlayer in the skin's colours rather than with the skin's own bitmaps, and the library falls back to a standard library window drawn in those colours. Skins are treated as untrusted input throughout — everything is read through a sandboxed virtual filesystem with enforced size, depth, and execution limits, and scripts cannot reach your filesystem, launch arbitrary applications, or make general host network requests; only an actual embedded browser object may load policy-gated web content.
...
- **Winamp 5.x modern skins: the Library's ART button no longer crashes the app** — inside a `.wal` skin, switching the Library browser to artwork view could quit nullPlayer outright the moment the star rating was drawn. A skin whose list-text colour resource is missing or unparseable fell back to plain white, which macOS hands over as a greyscale colour, and reading a red/green/blue channel off one of those aborts the process rather than returning a value. The skin palette is now kept in a colour space its channels can actually be read from, and an unreadable colour resource resolves to a real white.

- **Winamp 5.x modern skins: a skin's own pages scroll** — Big Bento Modern's settings pages are taller than the space they're shown in and carry their own scrollbar, but nothing below the fold could be reached by any means: the wheel didn't reach the skin, the scrollbar's arrows lit up and moved nothing, and the bar couldn't be dragged. Seven separate things were wrong at once, and the pages stayed stuck until the last was fixed. The wheel now reaches the skin (skins ask for it on the window as a whole and work out which panel the pointer is over, so a page that isn't under the cursor correctly ignores it); "scroll this page to here" now moves the page; and a scrollbar built as a wrapper around a slider — which is how Winamp skins build them — now behaves as one control instead of two halves that disagreed about where the bar was. Pages also open at the top rather than scrolled to their own end.

- **Winamp 5.x modern skins: vertical sliders work vertically** — a skin can write the direction of a slider two ways, and nullPlayer only recognised the long spelling, so 49 sliders across 8 skins were treated as horizontal ones. Their handles slid the wrong way, and dragging one read the mouse's *sideways* movement across a bar sixteen pixels wide, so it jumped to one end instead of following the pointer. This is why Big Bento Modern's settings scrollbar couldn't be dragged at all — and, unreported until now, why the equalizers in Anexa, Lobe and cPro Bento could never show a curve: all ten band controls slid sideways inside their own columns. They now travel up and down, and the equalizer draws the shape you set.

- **Winamp 5.x modern skins: the seek bar survives being used** — on Big Bento Modern, one click or drag on the position bar made it vanish, and because there was then nothing left to click you could not seek again until the next track started. The skin's own script hides its seek slider when you let go of it — harmless in the skin it was adapted from, where that line pointed at a small readout that isn't present, but here it pointed at the slider itself, and the skin hides the groove and the progress fill along with it. nullPlayer now notices when a script has left a window with no visible way to seek at all and puts the control back, which also brings the groove and the fill with it. A skin that simply swaps one seek control for another is untouched — the check runs after the skin has finished, not in the middle of it — and controls that are *meant* to come and go, like a play button making way for pause, are deliberately not covered.

- **Winamp 5.x modern skins: the song title comes back after you drag the seek bar** — on Big Bento Modern, dragging the position slider replaces the song title with a live `Seek: 1:13 / 4:05 (30%)` readout, and letting go was meant to put the title back. It stayed stuck on the readout instead. A skin can call one of its own event handlers directly instead of waiting for the event, and this skin uses that to tell its timer "run now, don't wait for the next tick". nullPlayer already allowed that for a skin's on-screen objects and for the player itself, but not for the timers and helpers a script creates as it runs — so the handler stopped at that line and never restored the title. It works on all three now.

- **Winamp 5.x modern skins: Big Bento Modern's seek bar is a seek bar again** — the strip under the time readout was a solid black bar with no track position in it. The skin reserves a box there for a waveform seeker that belongs to WACUP, a different Winamp-derived player, and marks that box as holding *nothing* when the plugin isn't present. nullPlayer read "nothing" as the name of some add-on it didn't recognise, and it fills an unrecognised add-on's box with a flat panel so you can see something is missing there — which laid an opaque slab over the seek bar underneath. A box that holds nothing now draws nothing, and the seek bar shows its trough and the coloured progress fill as the skin intended. The Windows 10 editions of the skin cover their seek bar for a separate reason and still show it blank.

- **Winamp 5.x modern skins: an element sized against its parent is sized against its parent again** — on Big Bento Modern the album cover appeared twice in the panel beside the transport controls: the real one, and a small crisp copy of it a little to the right. The copy is meant to be an oversized, dimmed version of the cover sitting *behind* the whole panel as a backdrop. A skin can size an element relative to its parent by setting a flag beside the width and height, and nullPlayer only recognised the value `1` for that flag — this skin writes `2`, and means the same thing by it. Anything else was treated as a fixed pixel size, so a backdrop asking to be as wide as the panel came out 99 pixels wide instead. Ebonite and the Nokia 5220 skin write other numbers in the same place and are corrected too, while two skins that write a literal `%` there keep their existing behaviour, which is what Winamp does with it.

- **Winamp 5.x modern skins: Big Bento Modern's 77 colour themes can be picked** — the skin ships 77 colour themes and a settings page listing every one of them, and clicking a theme did nothing at all. Winamp hands a skin a colour-theme manager as a second built-in object alongside the main one, and nullPlayer only recognised the main one, so the script that applies a theme stopped at its first line. It is recognised now, and picking a theme applies it through exactly the same path the rest of the player already used to change themes, so the two can never disagree about which theme is current. Ebonite also reaches its themes this way and gains the same fix. Picking a theme the skin does not actually ship does nothing instead of breaking the rest of the page.

- **Winamp 5.x modern skins: Big Bento Modern's settings pages and equalizer tab are built** — the skin's own options window offers nine pages, and its equalizer tab sits beside the player, and every one of them came up completely empty. In the skin's markup those pages really are empty: each is nothing but a scrollbar and a placeholder list, and the controls that belong on it are stored separately and inserted by the skin's script when the page is built. nullPlayer had no answer for "build this into that list", so the script stopped at its first attempt and nothing was ever inserted. It is answered now, including the part the skin leaves to the list itself — each block is widened to fill the page and placed below the one before it, rather than all of them landing in the same spot at zero width. With the pages finally running, the Localization page's script asked one more question nullPlayer could not answer — where the player is installed, which it uses to look for Winamp's own language packs — so that is answered too; the packs are not there, which is the truthful reply and the one the skin handles. This was also the last thing keeping all four Big Bento Modern editions marked as unsupported.

- **Winamp 5.x modern skins: Big Bento Modern's info panel comes to life, and a skin that defines the same startup step twice gets both** — the big panel to the right of the transport controls was mostly an empty dark box with a black square where the album art belongs, and the visualization slot was drawn on top of the cover instead of one or the other. All of it came from a single rule. A skin's script file can define the same startup step more than once, and nullPlayer had learned — from another skin that ships the *same* step twice by accident — to keep only the last one. Big Bento defines two genuinely different startup steps for that panel: one lays the whole thing out and decides whether it should show album art or a visualization, the other just starts a timer. Keeping only the second threw the entire panel away. nullPlayer now drops a repeat only when it really is a repeat of the same code, so both steps run and the other skin's double-fire stays fixed. With the panel finally running, the code that fills in its track details stopped on four more things nullPlayer could not answer — the file's format, its folder, its file name and, oddly, the size of a video that isn't playing — so those are answered too and every line of the panel now fills. Three older problems turned out to have been hidden behind the same rule and are fixed with it: the panel's details column sat squashed in a corner, a full-width cover-flow strip was drawn across it, and the shade-mode title bar was still printing "WACUP" over "WINAMP".

- **Winamp 5.x modern skins: the window stays draggable, and a skin's tabs stop stacking up** — two things that only showed up once Big Bento Modern was usable. Its window became impossible to drag after switching to the compact shade view and back: nullPlayer recognised a skin's "you can drag the window here" marking on only one kind of element, and this skin puts it on two others, so the window had only ever been draggable by luck — wherever a piece of plain background happened to be on top — and shade mode changed which piece that was. The marking is now recognised wherever a skin puts it, except on buttons and sliders, which keep their own clicks. Separately, a skin that packs its playlist, library, video and equalizer into tabs of one window could come up with two of them drawn on top of each other, if your last session had left both the playlist and the library open: nullPlayer opened each in turn without either closing the other, and the skin's own script — which picks a tab a moment after the window appears — had no idea a second one was open. Opening one now always closes the one it replaces, and where nullPlayer and the skin disagree the skin wins.

- **Winamp 5.x modern skins: Big Bento Modern draws its menu bar, its time readouts and its album art** — with the four Big Bento Modern skins loading for the first time, most of what they drew was wrong: the File/Play/Options/View/Help menu bar printed all five words on top of each other, the album-art panel was a black square, the time and length readouts either side of the song title were blank apart from the slash between them, the shade-mode title bar drew "WACUP" over "WINAMP", and the tab strip beside the player printed each tab's caption across its own icon. Nearly all of it came from one thing. Big Bento checks whether it is running inside WACUP, a different Winamp-derived player, by looking for one of that player's files — and it does this at the top of almost every one of its scripts. nullPlayer had no answer for "where do you keep your settings?", so those scripts stopped there, and everything they would have done afterwards — which is where this skin positions its menu bar, its cover art and its tabs — never happened. nullPlayer now answers, the check finds nothing, and the skin carries on down its "not WACUP" path. Separately, the skin asks for several playback readouts nullPlayer did not recognise by name — elapsed time, track length, title, sample rate — so those fields drew nothing; they are now filled in, which also fixes the KBPS/KHZ readouts on Ebonite and Enkera and the song length in MMD3's shaded playlist. The skin's own config window and equalizer tab still come up empty, which is a separate missing piece.

- **Winamp 5.x modern skins: the four Big Bento Modern skins load** — Big Bento Modern, its Light edition, its Windows 10 edition and that edition's Light version all refused to load, and each one was blocked by a different thing. All four address files through Winamp's *skins folder* rather than their own folder, a shorthand nullPlayer did not recognise, which failed the very first line of every one of them. The two Light editions are not whole skins at all: they ship a palette and borrow the rest of the artwork and markup from the edition they are based on, so nullPlayer now loads that base skin alongside them (up to four such companions, each read under the same safety limits as the skin itself). If the base skin is not installed, the skin no longer fails with a file path — it says which skin you need to install, by name, and installing it under its original filename is what makes it work. And the Windows 10 edition ships one empty image file by mistake, a placeholder for missing album art; an image a skin cannot decode now simply does not draw, instead of stopping the entire skin from loading. All four now open and draw; the Light editions come up in their own light palette.

- **Winamp 5.x modern skins: shuffle, repeat and crossfade look switched on when they are — and crossfade actually crossfades** — on MMD3 the Crossfade, Shuffle and Repeat buttons gave no sign of being on: shuffle and repeat worked, but the little lamp beside each button stayed dark and the word in the display stayed dim, and the crossfade button changed nothing at all. These three buttons are bound to Winamp's own playback settings rather than carrying a command of their own, and a skin draws them from that setting: MMD3 deliberately uses the same picture for the pressed and unpressed button, so the *only* indication is a lamp and a brightened word whose visibility its script sets from "is this switched on?". nullPlayer kept a bound button's on/off state on the button itself, where that question was answered "off" forever, and never told the skin when a switch moved — so no skin in the collection could show a toggle's state. Shuffle and repeat were also being stored in two places at once, the skin's copy and the player's, which drifted apart the moment either one changed. Those settings now live in one place, the player's, and the skin reads and writes it directly: the lamps and words light up, changing shuffle or repeat from the **Playback** menu lights them too, and MMD3's crossfade button and its duration slider drive nullPlayer's **Sweet Fades**, clamped to the same 1–10s range the Fade Duration menu offers. Measured across the 30 installed skins these are by far the most common settings any skin binds — repeat 52 times, shuffle 50, crossfade 32 — so this is a fix for the collection rather than for one skin.

- **Winamp 5.x modern skins: a skin doing arithmetic on a missing value can no longer take the app down** — a skin script that divided by a value that happened to be zero produced an infinity, and converting that back to a whole number crashed nullPlayer outright instead of failing safely. Nothing in the collection had reached it before; multipass did as soon as its crossfade slider started reporting a real position. Skins are untrusted input and are never allowed to crash the player, so out-of-range and undefined results now saturate or read as zero, as the rest of the script engine already did.

- **Winamp 5.x modern skins: a slider with its own range is driven in that range** — MMD3's crossfade slider is marked out 0–20 seconds and prints its position straight into the display, and Anaheim's brightness slider runs −4096 to 4096, but both were being handed Winamp's default 0–255 regardless of what they asked for. Sliders now report positions in the range they declare.

- **Winamp 5.x modern skins: a skin that paints its own artwork black, and a skin that tints real artwork, can both be right at once** — on Anaheim Player 01 the player came up almost entirely black: black text on black playlist and library windows, and the ring of controls that is supposed to appear when you hover the mini player never showed up, because those controls were being drawn black on a black body. Fixing it by brightening colour themes then washed out every skin that themes real artwork — Anexa and MMD3 among them — so each fix broke the other skin. Neither model was ever meant to apply to everything: a colour theme adjusts each channel by an amount, and every theme group carries an attribute saying whether that amount should be **multiplied** into the artwork or **added** to it, which nullPlayer was reading and discarding. Skins like Anexa ship finished artwork and ask for multiplication, so a theme tints it without bleaching it. Skins like Anaheim ship their artwork as pure black stencils with nothing but a shape, and ask for addition, so the theme is what puts every colour on screen — multiplied, black stays black no matter what the theme says. Winamp's own stock skin uses both inside one skin, multiplying its backgrounds and adding to exactly the pieces it draws black. nullPlayer now honors what each group asks for: Anaheim's text, controls and hover ring appear in the theme's colours, and every skin that was already themed correctly is untouched.

- **Winamp 5.x modern skins: a button that opens a tab works the first time you press it** — on Defix, pressing the Media Library round button did nothing at all: the library would not open until some other window had been opened and closed first, after which the same button worked every time. The skin's script opens its SUI window, then asks "is the library tab already showing?" to decide whether it should switch to it or close the window — and nullPlayer answered "yes" even though nothing was on screen, because it only checked the tab page's own visibility flag, not whether the tab page was inside a panel that was hidden. In Winamp, "is this visible?" walks up through every parent — an object inside a hidden panel is not visible — and nullPlayer now does the same. The same fix covers any skin that gates a tab switch or a panel reveal on the visibility of something nested inside a hidden group.

- **Winamp 5.x modern skins: a skin's playlist window shows the playlist** — on LOBE and on any skin that wires its playlist window the same way, the Playlist Editor window opened with the correct chrome — title bar, frame, minimize/close — but the body where the playlist rows should appear was blank. The skin tells its window frame which group to place inside it using a `notify="content,…"` attribute, a Wasabi shorthand for passing a named parameter to the window's script, and nullPlayer only delivered parameters on elements written with the XUI tag name (like `<Wasabi:StandardFrame:Status content="…">`), not on a plain `<group>` element used by its id. The frame script never learned what to put inside itself and never built the playlist area. Lobe, Anexa, Sony Walkman and boom all use the same pattern; playlist content now appears in all of them.

- **Winamp 5.x modern skins: a skin's knobs and dials answer the mouse again** — on LOBE the volume strip and the seek dial did nothing: dragging either one changed neither the volume nor the position. Neither control is a slider — each is an animated dial the skin's own script drives, and the script works out where you clicked by looking the point up in a small picture of the control. nullPlayer was telling the script where the click was **on the whole window** when Winamp tells it where the click was **inside the panel the control sits in**, so on any skin that puts a dial inside a placed panel the lookup fell outside the picture entirely and came back as zero — the dial seeked to the start of the track and the knob turned the volume to nothing. Styx's volume knob was broken the same way and is fixed by the same change. Skins whose dials happen to sit at the corner of their window — the stock-style MMD3 knobs among them — were never affected and behave exactly as before.

- **Winamp 5.x modern skins: a skin's compatibility report stops crying wolf** — every `.wal` skin carries a compatibility report, and for most skins it was mostly noise. A skin that shares one file of artwork definitions between two of its windows includes that file twice — ordinary practice, and what Winamp itself expects — and nullPlayer logged a warning for every definition it read the second time, even though the two were identical. On LOBE that was 198 of its 233 findings. A definition read twice is now only reported when the second one actually says something different, which is the case worth knowing about; across the thirty skins used to measure this mode the reports shed 492 of their 1343 entries, and every genuine redefinition is still listed. Nothing about how a skin loads or draws changed — the later definition wins exactly as before.

- **Winamp 5.x modern skins: a skin whose window has several named shapes now opens** — a modern skin's window is described by one or more *layouts*, and nullPlayer only knew how to open the layout named `normal` (or a lone unnamed one). A skin whose window offers several shapes names them for what they are instead — BLAKK's player is `boombox` / `remote` / `stick`, Ebonite's is `full` / `compact` / `mini` and three more — and neither of those two skins would load at all: selecting one left an empty placeholder where the player should be. The same rule quietly dropped LOBE's About window, whose six pages hold the skin's 43 colour themes and the button that applies them, so that whole screen was unreachable by every route. A window with no `normal` layout now opens in its **first** layout, which is what Winamp does, and a container nullPlayer genuinely cannot open is written into the skin's compatibility report instead of disappearing without a word.

- **Winamp 5.x modern skins: buttons drawn in faint or glassy artwork now take a click** — on a skin whose controls are drawn semi-transparent, most of the player was inert: on LOBE, thirteen of the twenty-three controls in the main window did nothing when clicked, including previous, stop, close, minimize, the playlist and media-library buttons and the shuffle, repeat and crossfade toggles. The skin draws each button as a glassy disc at about a third opacity with its icon engraved into the disc as an even fainter shape — and nullPlayer decided whether a click had landed on a button by asking whether that exact pixel was solid enough, using a threshold written for artwork drawn at full opacity. The icon in the middle of each button was below it, so a click aimed at the icon fell straight through to the background behind it and the window read as dead rather than as a button that had missed. A click now lands anywhere the skin actually painted something, which is the rule Winamp itself uses. Buttons that were already working are unaffected, and a skin's hover and pressed overlays still let clicks through to the control beneath them.

- **Winamp 5.x modern skins: the visualization and library windows no longer open taller than the screen** — on a skin whose own window is tall, opening nullPlayer's visualization window from the skin (or its library window) for the first time produced a window roughly four times the height of the skin itself, running off the bottom of the display: on LOBE, whose player is 300pt tall, that is 1200pt, and twice that again at 200% UI Size. The default height came from a rule written for the classic skin's main window, which is a thin strip about a hundred points tall, so multiplying it by four gives the familiar column beside a stack of windows; a modern skin's window is its own full-size canvas, and the same multiplication gives nonsense. A modern skin's side windows now open at that same familiar column height whatever size the skin's own window happens to be, and any first-time window is additionally trimmed to what the display can actually show. Classic and Modern skins are unaffected — the number they get is unchanged.
...
- **Winamp 5.x modern skins: a skin's visualization window shows the real visualization** — eight of the modern skins nullPlayer has been measured against draw a window for Winamp's visualization plug-in — the window AVS and MilkDrop filled — and it held a spectrum analyzer: the same bars as the little visualization box in the player, only bigger. It now holds nullPlayer's own visualization engine, so **ProjectM/MilkDrop, Geiss and Tripex play inside the skin's own window**, in the box the skin drew for them, using the engine, presets and settings you have already chosen. The window is now listed in **Skin Windows** and opened by **Show Visualizations Window** — until now there was no way to open one at all, since no skin binds a button to its own visualization window — and the skin's own visualization buttons drive what is running: Previous and Next step presets (or effects, for Geiss and Tripex), and the menu and options buttons open the engine's controls, as does a right-click on the picture. That right-click menu is the **same full menu** the visualization window has — preset list with ratings and favourites, auto-cycle, engine choice, Geiss and Tripex settings — rather than a shorter one of its own. The **Fullscreen** button inside a skin's visualization window fills the screen with the visualization itself (Esc, F or a double-click brings it back) instead of opening a second window, and only ever one visualization window is open at a time — nullPlayer's or the skin's, never both competing over the same audio. Two skins whose visualization window is declared narrower than the size its own artwork is drawn for (Anaheim Player, Styx) no longer open with their frame cut off down one side. The visualization menu's engine choice drives the skin's window and nullPlayer's together, so the two can never disagree about what is playing. The small visualization box in a skin's player is a different thing and is unchanged — it is Winamp's own analyzer and oscilloscope, drawn in the skin's colours. Skins with no visualization window of their own still use nullPlayer's, exactly as before.

- **Winamp 5.x modern skins: a skin's video window plays the video** — fifteen of the modern skins nullPlayer has been measured against draw a complete video window — a frame, a status readout and a row of Fullscreen / 1x / 2x / TV / Options buttons — and it was a picture frame around an empty black box. Play a video and nullPlayer's own video window opened somewhere else on screen while the skin's stayed shut, exactly as if the skin had never drawn one. The video now plays **in the skin's window**, filling the box the skin drew for it: the window opens when a video starts and closes when it stops, the skin's own buttons drive it, and the picture follows the window as you move and resize it. Its **1x** and **2x** buttons work for the first time — they size the window so the picture is the video's own size, or double it, stopping at the edge of your screen — and **Fullscreen** takes the video out to a full screen and puts it back in the skin's window afterwards. Nothing about casting changed: send a video to a TV and no local window opens, skin or otherwise. Skins with no video window of their own are unaffected and still use nullPlayer's.

- **Winamp 5.x modern skins: a skin's own window-scaling buttons work** — Defix's configurator offers seven sizes for its windows, from 100% up to 300%, and pressing any of them did nothing at all. They now change nullPlayer's **UI Size** — the same setting as the UI Size menu, so the skin's buttons and the menu are two ways of turning one dial rather than two rival scales that disagree about how big the windows are. Three new sizes came with them: **175%**, **250%** and **300%** are now offered in the UI Size menu in every skin family, so each of the skin's seven buttons lands on its own size instead of three of them collapsing onto 200%. A size you have set yourself and had restored at launch still wins over the one the skin remembers.

- **Winamp 5.x modern skins: a skin's keyboard shortcuts work** — a modern skin can define its own keyboard shortcuts, and none of them did anything: multipass and the stock Winamp Modern skin both use **Alt+G** to open and close their equalizer drawer, the Winamp Modern skin also uses **Ctrl+W** to fold the focused window into its compact winshade form and **Alt+A** for its album-art panel, and Defix uses **Esc** to close its playlist search box. Two things were in the way. A skin's window has no title bar, and a window with no title bar is never given the keyboard by macOS, so the key press was not merely ignored — it never arrived. And a skin's handler asks "does *my* window have the keyboard?" before acting, a question nullPlayer had no answer for, which quietly killed whatever the script was doing. Both are fixed: keys reach the skin, Ctrl+W folds the window you are actually looking at rather than all of them, and a key the skin does not claim still falls through to nullPlayer, so ⌘Q, ⌘W and every other menu shortcut behave exactly as before.

- **Winamp 5.x modern skins: a skin's playlist window and its own playlist controls work** — a modern skin reads and edits the playing queue through Winamp's playlist-editor object, and nullPlayer answered none of it: the queue's length, which track is playing, an entry's title, length, filename or album, and the commands behind a skin's playlist context menu (play, remove, move to top or bottom, clear, scroll to the playing track). Defix's playlist window used this to look up the album art for the entry under your mouse and to drive its Move/Remove selected commands; the stock Winamp Modern skin's, cPro-Bento's, Itemskin's, Love is War Miku's and multipass's track counters read it too. The cause was one line: Winamp's compiled scripts mark the playlist editor as a host-provided object in exactly the same way they mark the general "System" object, and nullPlayer had been treating both as System — so every one of these calls went to the wrong place and quietly killed whatever the script was in the middle of doing. Since it only ever failed on interaction, never at load, it read as an intermittent fault. All of it now answers.

- **Winamp 5.x modern skins: a skin button no longer opens a window and shuts it again** — pressing Defix's round playlist button made its playlist window flash open and close in the same click, and a window that was already open refused to close. The skin's compiled script declares that button's click handler twice, byte for byte, and nullPlayer ran both — so the button's action, which is a toggle, fired twice and the two cancelled out. Winamp keeps one handler per control per event, and nullPlayer now does too. Any skin whose author left a duplicated handler behind is fixed by the same change.

- **Winamp 5.x modern skins: a skin's equalizer readout follows the equalizer** — five of the seventeen `.wal` skins used to measure this mode draw their own equalizer display — multipass's eleven LED bars, the stock Winamp Modern skin's drawer, Rika's, mmd3's and Overdrive_2's — and each of them was told about a change only when you dragged the skin's own slider. Apply a preset, use the equalizer window, or start nullPlayer with an equalizer you set last session, and the skin's bars sat wherever they happened to be, disagreeing with the sound. Every route now tells the skin: a preset, the menu bar, the equalizer window, the skin's own scripts, and the state restored at launch, so the display shows the equalizer you are actually listening to, from the moment the skin opens.

- **Winamp 5.x modern skins: the windows a skin opens with itself now open** — a modern skin can mark any of its own windows as one that comes up *with* the skin, and nullPlayer ignored that entirely: every extra window started closed, however plainly the skin asked otherwise. Defix's configurator — the panel with its 31 backgrounds, display styles and window scaling — and its playlist editor both start on screen now, as they do in Winamp, and so do the stock Winamp Modern skin's playlist and album-art panels, Ujola Cat's cat window and playlist, ZDL's equalizer and thinger, and Overdrive_2's playlist. They also open **where the skin puts them** — beside or below the player, in the arrangement the skin ships — instead of stacking up under it. This is a default, not a rule: close one and it stays closed for that skin, on this and every later launch, so nothing you have dismissed comes back uninvited. Two kinds of window are deliberately left shut even when a skin asks for them, because nullPlayer has nothing to put in them: Winamp's pop-up track notifier (which here would be a permanent box reading "Nothing"), and the "home page" browser windows in Rika and T800, which would open as an empty 860×704 frame — both are still one menu item or one skin button away, and each records why it stayed shut in the skin's compatibility report.

- **Winamp 5.x modern skins: the toolbar buttons on a skin's playlist, visualization and video windows work** — a modern skin's windows carry the same small toolbars Winamp's do, and pressing any of them did nothing at all: eleven of the seventeen `.wal` skins used to measure this mode draw them, 108 buttons in all. All of them are now answered. The **playlist** window's five buttons open the menus they do in Winamp — **Add** (files, a folder, a URL), **Rem** (remove the selection, crop to it, remove everything, remove dead files), **Sel** (select all, none, invert), **Misc** (sort by title, filename or path, randomize, reverse, file info) and **List** (new, load or save an .m3u) — all driving the same playlist the rest of nullPlayer uses, and a skin's own embedded playlist now highlights every selected row rather than only the last one clicked. The **visualization** window's buttons step through nullPlayer's visualization presets when that window is open, and otherwise switch the skin's own in-window visualization between spectrum analyzer, oscilloscope and off; its menu button also carries nullPlayer's own Visualizations menu, and its fullscreen button opens the visualization window fullscreen. The **video** window's fullscreen button and its options button (which opens the video window's own menu, audio and subtitle tracks included) work whenever a video is playing. Two kinds of button stay deliberately inert because nullPlayer has nothing behind them — Winamp's Internet TV, and the arrows that scroll a strip of Winamp plug-in icons that does not exist here — and each now records why, so it shows up in a skin's compatibility report instead of looking broken.

- **Winamp 5.x modern skins: text sits where the skin asked it to sit** — a modern skin can say where a line of text should sit inside its own box (top, middle, or bottom), and nullPlayer ignored it: everything was centred. Nine of the seventeen `.wal` skins used to measure this mode say otherwise, in 63 places — the Nokia 5220 skin's entire screen, multipass's display, Defix's song ticker and info line — so their readouts floated in the middle of slots drawn for text sitting on the top line. Text drawn with a skin's own pixel-art font moved the opposite way: it was always pinned to the top of its box, whether or not the skin asked for that, which lifted readouts (and the track rows in a skin's own playlist) half a slot too high. Both are fixed, and a string too tall for its box still starts at the top of it rather than above it.

- **Winamp 5.x modern skins: the balance slider works** — seven of the seventeen `.wal` skins used to measure this mode draw a balance slider, and every one of them was a control you could drag with nothing at the other end: the thumb snapped back to hard left and the sound never moved. Dragging one now pans the audio left and right through nullPlayer's own balance, and the thumb is drawn from that balance rather than from the skin's markup — so it starts centred, and it follows a balance changed from anywhere else. A skin's own readout came back with it: sliders in these skins tell the skin's scripts where they were dragged to, which is how multipass flashes "Balance: Left +40%" on its song ticker and Ujola Cat's right drawer reads out `Balance: 30% Left` — that message was only ever sent by a script before, never by your mouse, so every slider readout in every skin was silent. Balance is not remembered between launches, in the modern skins as everywhere else in nullPlayer.

- **Winamp 5.x modern skins: double-click and right-click on a skin's own controls now do something** — a modern skin can put one command on a click, a second on a *double*-click and a third on a right-click, and nullPlayer only ever read the first. Nine of the seventeen `.wal` skins used to measure this mode were affected, 62 places in all. Two things came back: **double-clicking a skin's title bar switches it to its compact "winshade" form and back**, the way it does in Winamp (MMD3, multipass, the stock Winamp Modern skin, ZDL and Overdrive_2 all spell it that way, for their player, playlist and equalizer windows alike), and **the song title responds again** — double-click it for a File Info panel about what's playing, right-click it for a small track menu (File Info, Copy Title, Reveal in Finder). Song titles were doubly dead: nothing read the attribute, and because a title has no ordinary click action the mouse fell straight through it to the artwork behind. The main-menu "≡" button is unchanged and still opens nullPlayer's own menu.

- **Winamp 5.x modern skins: the two skins that would not open at all now do** — of the seventeen `.wal` skins used to measure this mode, two (Itemskin and Overdrive_2) failed to load outright and showed an error instead of a player. Each names one XML file in its own package that the package doesn't actually contain — Winamp shrugs at that and carries on, and nullPlayer now does the same, skipping the missing piece and noting it rather than throwing the whole skin away. (A skin that reaches for the *ClassicPro plugin* and can't find it still fails with a clear message, because a skin missing its engine would otherwise open as a nearly empty window.) Behind that, Overdrive_2 needed two more fixes: a skin script nullPlayer can't read is now skipped instead of taking the skin down with it, and one of Overdrive_2's five scripts turns out to be written in an older, undocumented flavour of Winamp's compiled-script format from 2001, which is now read as well — so the skin's seek bar, clock and equalizer scripts all run. Both skins now draw: Overdrive_2's speedometer face and transport ring, Itemskin's player, mini and equalizer layouts.

- **Winamp 5.x modern skins: the multipass skin comes alive — drawers, seek bar, clock, sliders and its 58 colour themes** — the multipass skin opened as a static picture: no sliding drawers, a dead seek bar and time readout, dead volume and balance sliders, no notifier, and a Style menu that changed nothing. All of it was one missing script call. The skin does its entire setup in a single startup routine, and the eighth line of that routine asks Winamp to build a floating panel for its colour-theme list — a call nullPlayer did not implement, and a call a skin script cannot survive, so everything after that line (which is to say, the rest of the skin) never ran. That call and five more the skin needs are now implemented, and multipass comes up complete: hovering the player slides its volume and left drawers out, the left drawer's toggle opens the bottom drawer with its equalizer, options and colour-theme pages, and the colour-theme page shows a populated 58-row list where an empty box used to be. The Style menu swaps the skin's artwork as designed, and **its seek bar is finally
there**: it was invisible (a bar drawn 28 times too tall, then clipped away to nothing), unclickable
ahead of the playhead, and — because whole-number division in the skin-script interpreter threw the
fraction away — every click on it sought to the very beginning. All three are fixed, so the bar shows
the position, the whole width takes a click, and the display reads out the time you are about to jump
to. That division fix reaches other skins too: cPro-Bento's readout said `Volume: 0%` at every level
and now reports the real one, and MMD3's volume bar and the stock Winamp Modern skin's volume slider
fill in properly. Along the way, a togglebutton anywhere in any `.wal` skin now tells the skin's scripts when you click it — before this it only ever changed from a script, so a button a person pressed could sit there doing nothing however completely the skin implemented it. The `≡` button in the top-left corner of a skin's title bar — Winamp's main menu button — now opens nullPlayer's own menu instead of doing nothing, in every skin that draws one (multipass, CornerAmp Redux, Overdrive_2, the stock Winamp Modern skin, ZDL, MMD3). A note in nullPlayer's own notes claiming multipass "ships no colour-theme list" was our bug, not the skin's, and has been corrected.

- **Winamp 5.x modern skins: colour themes can finally be picked** — a modern skin ships colour themes (MMD3 has 82, multipass 58, the stock Winamp Modern 44) and a screen to choose them from, and every one of those screens was an empty box that couldn't be clicked: the list widget itself lives inside Winamp, so the skin only names it and nullPlayer drew nothing where it should be. Those lists now populate with the skin's own theme names in the skin's own order, colours and font, with the theme you're wearing marked and the list opening scrolled to it — click a row to pick it out, double-click to apply it, and scroll with the wheel. The **Switch**, **Next** and **Previous** buttons skins put beside their list work too, including the ones whose list lives in another window, and a Switch button that names no list (Defix's, multipass's) opens the same list as a popup menu instead of doing nothing. Skins that define themes and ship no picker at all — in Winamp those are chosen from Winamp's own preferences dialog — get a new **Color Themes** submenu in the Winamp Modern menu, with the applied theme checked. Switching from any of these routes recolours the player, the playlist, the equalizer and the embedded library together, and the choice survives a relaunch. Plain text buttons a skin draws with no artwork of its own (the "Switch" under three skins' theme lists) are now drawn as a bordered label rather than being invisible.

- **Winamp 5.x modern skins now actually draw** — the experimental `.wal` mode loaded skins correctly but rendered them wrong in ways that made them unusable: every sprite was cut from a mirrored position in the skin's image sheet, each one was then drawn upside-down, resizable pieces (window frames, the menu bar, the title bar) painted a single small tile and left the rest of the bar blank, and groups that fill their parent collapsed into the top-left corner. Winamp Modern's window body was missing entirely because the skin builds its own client area at runtime through a script call that was a silent no-op, and its menu titles stacked on top of each other because the menus reported zero width. The song display now shows the artist and title, scrolls when the text is too long, and fills in bitrate and sample rate. cPro-Bento and Winamp Modern both render their frames, displays, and controls.
- **Winamp 5.x modern skins: correct colors, working drawers and knobs, and UI Size support** — skins with color themes (MMD3 and anything else built on `gammaset`s) came up washed out and in the wrong theme: a theme's per-channel values were applied as a brightness offset instead of a multiplier, and the default theme was chosen alphabetically rather than being the skin's own first one, so MMD3's amber display rendered as pale green. Skin scripts also stopped partway through startup — a handful of missing script capabilities (bitmap "maps", animated-layer frame ranges, equalizer band access, cursor position) aborted the very setup pass that wires up a skin's sliding drawers, rotary volume/bass/treble knobs, LEDs, and menus, so none of them responded; a call on an unset object now does nothing instead of taking the rest of the script down with it, as in Winamp. Objects a skin parks off-screen to hide them no longer leak their artwork into the window. Winamp Modern windows now honor **UI Size**, so a skin drawn on its original small pixel grid can be scaled up like the other skin families.
- **Winamp 5.x modern skins: ClassicPro skins (cPro-Bento) now run their scripts** — cPro skins drew their window frame and nothing else, with a dead body where the playlist and library belong. The skins' engine ran almost none of its own code: nine missing script capabilities aborted thirteen of its startup scripts, including the main player, the menu bar, the visualization setup and the single-window layout script — and each script that aborts loses everything it had yet to wire up. Underneath that were two deeper faults. `delete` (which a script does after reading a colour or a size out of a bitmap) corrupted the script's own working stack, killing any script that used it. And the engine's "is the ClassicPro plugin installed?" check reads a marker image *by path*, which only worked for images the skin declared by name — so the skin concluded the plugin was missing and tried to switch itself away before building anything. cPro-Bento now completes startup with no script errors: the beat visualization, spectrum, stream-info readouts and the window frame all come up, and a skin that ships no optional frame artwork no longer has holes punched through its edges. The single-window body (library tree, playlist, tabs) is still empty — that content is hosted by a Winamp built-in that remains unimplemented.
- **Winamp 5.x modern skins: ClassicPro skins (cPro-Bento) now build their body — tabs, playlist and menu bar** — cPro skins ran their startup scripts but still drew an empty body, because the single element their whole layout hangs off — a splitter that names the two groups it should place either side of it — did nothing. The splitter now creates and lays out both panes, so the tab strip (Media Library / Playlist / Video / Visualization / Browser), the playlist pane with its buttons, and the album-art area all appear, and a skin script can open and close the side view as designed. Reaching that code exposed a bytecode-reading fault that failed the *whole* skin rather than one script — a script variable declared as an object rather than a number was misread — plus a set of missing script capabilities (list and flag-array containers, reading Winamp's own preference groups, counting a group's children). Skin-driven text now measures the same way it draws, so labels sized by a skin's own scripts fit their boxes: the menu bar (File/Play/Options/View/Help) and the SUI tab labels render at full width instead of clipped or invisible. The stock Winamp Modern skin also benefits. The selected tab's content is still empty — the library surface remains hosted by the standard library window.
- **Winamp 5.x modern skins: the playlist, equalizer and library are now the skin's own** — opening any of the three from the Windows menu built a standard nullPlayer window, even when the skin already had one of its own on screen: cPro-Bento drew a playlist in its window *and* opened a second one next to it, and its Media Library tab was a frame around nothing. All three now resolve through one route, so the menu item and the skin's own button always reach the same place — the surface the skin embeds if it has one, the window the skin declares if it declares one, and otherwise a window built from the skin's own frame artwork so a skin that ships no library window still gets one that looks like it belongs. **cPro-Bento's Media Library tab now hosts the real library browser** — servers, tabs, search, Cover Flow, history, and linking, with no window chrome of its own. Skin equalizer controls finally reach the audio: dragging a band or the preamp changes the sound, the on/off button turns processing on and off rather than opening a window, presets apply from the skin's own presets button, and the sliders follow a change made anywhere else. The embedded playlist now draws in the skin's colours and font, shows the item-count/total-time line, and removes the selected row with Delete. Colour themes recolour every window of a skin at once instead of leaving the others behind, and a window restored smaller than the skin's own minimum is corrected instead of scrambling its contents into an overlapping pile.
- **Winamp 5.x modern skins: windows in CornerAmp-style skins now show their own titles** — a skin builds its title bar from one of Winamp's own standard-library pieces, which the skin file doesn't contain, so CornerAmp Redux's Playlist, Video, Equalizer and Library windows all came up with a blank title bar. Those windows now draw their name in the skin's own title colour. Skins that ship a title bar of their own (Winamp Modern, MMD3, cPro-Bento) are untouched. A tooltip piece that MMD3 asks for is also recognized now instead of being dropped with a warning.
- **Winamp 5.x modern skins: skin windows stop shrinking into an overlapping pile, and splitters can be dragged** — a `.wal` skin declares its own smallest window size, but those numbers are written for Winamp, which clips every group to its parent; nullPlayer clips only where a skin asks it to, so a window dragged small enough left pieces painting over each other — cPro-Bento's tab strip landed on top of its transport controls well above the size the skin says it supports. A skin window now refuses to go below the size at which its own layout still fits, measured from the skin itself, and that floor applies to dragging the window, to a size a skin script asks for, and to a window restored from the last session. Splitters — the movable divider between a cPro skin's tab pane and its playlist — can now be dragged: the divider takes a resize cursor, respects the limits the skin sets for it, and everything hosted inside the panes (including the embedded library browser) follows.
- **Winamp 5.x modern skins: the library is there when the skin opens** — a skin that hosts the library inside its own window (cPro-Bento, which starts on its Media Library tab) came up showing an empty pane, because that content was only built once you chose Windows → Library Browser. It is now shown as soon as the skin loads. Skins that use a separate library window are unaffected — nothing opens on its own at launch.
- **Winamp 5.x modern skins: the playlist, equalizer and library windows now match the skin instead of looking like a classic skin** — the library embedded in a `.wal` skin's own window, and the playlist / equalizer / library windows nullPlayer opens when a skin declares none of its own, were painted with classic-skin artwork: sprite-sheet window frames and buttons, the chunky 5×6 bitmap font, and list colours taken from whichever classic `.wsz` skin happened to be selected — a skin you aren't even looking at while a modern skin is on screen. They are now drawn flat and take every colour from the loaded modern skin itself, through the same colour resolution and colour-theme handling the skin's own artwork uses, so switching a skin's colour theme recolours them along with everything else. Text is a matched proportional font in place of the bitmap one. Nothing moves: every control sits exactly where it did, so window sizes, clicks and drags behave identically. Classic and Modern skin modes are completely unaffected.
- **Winamp 5.x modern skins: MMD3 gets its text, its knobs and drawers, and its own display back** — MMD3 came up with no song title, no time, no KBPS/KHZ, dead rotary knobs and dead drawers, and nullPlayer's spectrum bars painted over the skin's own animated display. Four causes. Skins name a bitmap font's image either by the name of a declared image *or* by its path inside the skin; only the first form was understood, and a font whose image never resolves draws nothing at all — so every string MMD3 draws disappeared, silently. Its display fields were bound to the wrong values: the song line dropped the artist, and the stream-info line (which the skin's own script reads back and picks the bitrate and sample rate out of) was handed the artist and album instead. A skin's placeholder text ("updating songticker") was treated as an override rather than as what to show when there's nothing else, so it never went away. Clicks over the drawer tabs, the colour-theme strip and the EQ tabs were being swallowed by the invisible full-window group MMD3 declares on top of everything — a group has no clickable area of its own in Winamp, only its contents do — and the rotary volume/bass/treble knobs are animated layers, a type that was never offered a click at all. Finally, a skin says which visualization it wants in a given box, including "none, I'm drawing my own here", and that was ignored; MMD3's animated display is now unobstructed, and its analyzer and oscilloscope modes still work.
- **Winamp 5.x modern skins: skins that size their windows from their artwork are no longer squashed into a pile** — a `.wal` skin can leave the size off a window layout and let its background artwork say how big it is (ZDL's Reel-To-Reel Analog Tape Machine writes every one of its windows that way), and those skins were being given the classic 275×116 player size instead. Everything the skin drew below that line — on the reel-to-reel, the whole tape deck under the two reels — fell outside the window and was dropped, leaving the rest stacked on top of itself. Such a window now takes the size of its own background art, so the skin comes up at its full height with its controls where the author put them. The same fault made these skins look like they shipped no equalizer window, so a plain one was built over the top of the skin's own; the skin's equalizer (and any other window it sizes this way) now opens as designed. Skins that state their window sizes outright — Winamp Modern, MMD3, CornerAmp Redux, cPro-Bento — render byte-identically.
- **Winamp 5.x modern skins: skins that reuse one group name for two layouts are no longer dead, and every skin window can be dragged** — a `.wal` skin can define a group, use it, then define a *different* group under the same name for another layout; T800 builds its full player and its shade bar that way. Only the last definition was being kept, so the full player was built from the shade bar's controls — nearly all of which landed outside the window and were dropped, leaving every button in the skin dead except the one that happened to have a unique name. Each layout now gets the group that was written for it, so T800's transport, equalizer drawer, playlist and media-library buttons, memory buttons and shade switch all work. Window dragging also no longer depends on a skin happening to put a movable layer under the pointer: the window can be dragged by its own background, which is the only handle skins like T800 have, and a piece the skin marks as fixed (T800's volume strip, which its script drags) no longer steals the window along with it. Minimize and close buttons a skin draws for itself now work too. The four reference skins — Winamp Modern, MMD3, CornerAmp Redux, cPro-Bento — and ZDL's Reel-To-Reel render identically.
- **Winamp 5.x modern skins: controls that fill or empty as a shape now work, and fixed-size skins stop stretching** — a `.wal` skin can clip one control to an arbitrary shape read out of a greyscale "map" image, which is how skins build a volume or seek bar that fills up as the value rises. Nothing behind it existed, so the call failed and took the rest of the skin's handler down with it: on T800 the volume strip never filled, and the "Volume: 40%" flash the same handler writes to the song ticker never appeared either. Regions are now built and applied, including the shorter one-call form MMD3 uses, and a map a skin ships broken degrades to an unclipped control rather than an invisible one. Separately, a skin window that states no resizing limits at all is now fixed at the size its author drew, exactly as in Winamp — T800 is a tall, narrow window whose entire face is one piece of art, and a window size left over from a previous session was stretching that art across it on launch. Skins that do state limits (cPro-Bento, Winamp Modern, MMD3, CornerAmp Redux) resize exactly as before.
- **Winamp 5.x modern skins: display text sized and placed as the skin drew it, working volume buttons, a seek indicator, and the skin's own right-click menus** — a `.wal` skin's on-screen text was a quarter too big and in the wrong typeface, so it overflowed the boxes the skin drew for it: the song line's descenders sat on the seek bar and the big time readout collided with the title. A skin's text size is a pixel height rather than a point size, a font it names without shipping is one it expects the system to have ("Arial"), bold and italic are separate settings, and text is centred in its box rather than hung from the top edge — all four are now honoured, and a clock that asks for fixed-width digits gets them, so the time no longer shuffles sideways as it ticks. The seek bar was an empty white box: skins draw the elapsed position as a filled bar and give the slider an invisible one-pixel thumb, and that filled bar was not being drawn at all, so nothing in the window showed where you were in the track. **A skin's own right-click menus now open** — previously no modern skin could show one, because the menu was built and then never presented, and any menu with a submenu failed outright; the visualization presets, colour and falloff menus that skins hang off their display all work, with the current choice ticked. **Volume controls work**: skins commonly drive their +/− buttons entirely from a script, and two faults stopped that dead — a set of missing value conversions aborted the handler, and beneath it every fractional number in every skin's scripts was being read as a small fraction of its real value, so the handler ran to completion and moved the volume by nothing. Click-and-hold now repeats and accelerates, and the skin flashes the level on its song display as it changes. The spectrum-analyzer and oscilloscope modes were also the wrong way round, so a skin's menu drew the opposite of what you picked and skins came up in the wrong one. Finally, controls a skin makes deliberately invisible (a click target over its display) can be clicked instead of dragging the window.
- **Winamp 5.x modern skins: cPro-Bento's scrambled window, its disappearing visualization, and its dead tabs** — the ClassicPro-based cPro skin came up garbled: a black slab across the top-right corner covering the volume slider, the mute button and the kbps/kHz readouts, a stray button floating over the display, and every framed surface — the tab pills, the sheet around the tab content, the playlist box, the mini-view strip, the seek track — drawn as a flat black hole. Three causes. The nine-piece stretchable frame element these skins build all of their chrome from was not implemented at all, so 49 of them in this skin drew nothing whatsoever; flat colour fills and colour fades were missing for the same reason. The top-right damage came from the *playlist* column, not the volume area: with the skin's mini view closed, that pane is a few pixels tall while its contents are anchored for the full-height strip, so they resolved above the pane and painted over the display — a pane of a splitter now clips its contents, as it does in Winamp. And a colour a skin defines as a solid one-colour image rather than as a plain colour resolved to **white**, which is what put a white bar across the tab strip of a near-black skin. **The visualization no longer vanishes when you press play** — the skin only enables its beat display from a window-resize notification that was never sent, so the first thing playback did was hide it with nothing able to bring it back; resize is now reported to a skin's scripts (as are pause, resume, track change, visibility changes, double-clicks and script teardown), and playback reports the actual transition instead of sending "play" for a resume. **The tab names now switch tabs** — clicking Media Library / Playlist / Video / Visualization / Browser had never done anything in any version. A skin builds those tabs at runtime in two steps, create and then place, and the "place" call did nothing, so each tab was left in the wrong parent and its clicks were addressed to an object that wasn't listening; script-to-script messages had no route at all; and a script asking how wide its own tab strip was got the raw markup value (a negative number) instead of the real width, so it squeezed every tab down to its minimum. Tabs now also draw inset inside the strip, as the skin's own reference screenshot shows. **The stock Winamp Modern skin's equalizer drawer opens for the first time** — the CONFIG button at the bottom right of that skin's window did nothing, and there was no other way to reach its equalizer. The button's handler was running and then aborting partway on a handful of window-management calls nullPlayer doesn't implement, and a skin script that hits a missing call loses everything it had left to do. Those calls are now accepted (as the no-ops they are for us, since nullPlayer positions these windows itself), so the drawer slides open with the equalizer — preamp and ten bands with the dB scale, on/auto/presets — the crossfade controls, and the EQ / Options / Color Themes tabs. **The logo that replaces the beat visualization no longer jumps sideways** — double-clicking the middle of the display swaps the animated beat display for a ClassicPro logo, and the logo landed well right of where the animation had been, further off the narrower the window. The skin centres it twice over, once when placing its box and again when placing the picture inside that box; the box is now shifted back by that inner offset, so both land on the same spot at every window width. **Closing the playlist column is no longer a one-way door** — the arrow that closes it collapsed the column and then left you stuck, because the skin swaps in a separate *open* arrow from a notification that was never sent, and because a pane that is hidden had no size at all in nullPlayer, so nothing could ever decide it was wide again. Skins are now told when their own actions move something — which also means dragging the divider re-flows the tab strip, and a skin's tabs switch between long and short labels as the space allows, the way they do in Winamp. A skin's spectrum box also repaints reliably from live audio rather than depending on the skin happening to contain an animation, and `thin`/`wide` band widths are honoured. Seven other reference skins render byte-identically.
- **Winamp 5.x modern skins: the close and minimize buttons work, and extra skin windows no longer open in the corner of the screen** — a skin's own Close button did nothing in any `.wal` skin, and Minimize with it. These windows have no system title bar, and the call behind the button asks macOS to *click the window's close button* — there isn't one, so nothing happened. Both now do what they do in Winamp: closing the player window quits nullPlayer (the same as the classic skin's close button), closing a playlist or library window the skin opens closes just that window, and minimize sends the whole set of the skin's windows to the Dock together. Separately, every extra window a skin opens — its playlist, library, video or visualization window — appeared at the very bottom-left corner of the screen, because it was never given a position. They now open stacked underneath the player window and stay wherever you move them.
- **Winamp 5.x modern skins: the stock Winamp Modern skin's window title is centered again, with its title bar streaks either side** — the two decorative streaks that flank the window name sat in a fixed slot near the left edge and never moved, and the skin's title bar script — the code that positions all three pieces — stopped partway on two missing coordinate conversions, so at anything but the default width the title drifted out from between them. Both streaks are two copies of one piece of the skin, told apart only by an instance name that was being ignored, so the script could never find either one to move it. It can now: the title sits centered at every window width with a streak running from each side up to it, and this skin no longer reports any load errors. The same two conversions were the last thing missing for **cPro-Bento's own right-click menus that a script places at a computed point** — right-clicking a tab now opens its Show Status Bar / Auto Close Tab menu directly under the tab.
- **Winamp 5.x modern skins: a skin that wouldn't load at all, and a spectrum painted over the skin's own display** — a modern skin that names a TrueType font it doesn't actually ship failed to load outright, with nothing on screen (Rika does this); a missing font is now treated like any other missing optional piece — the text draws in a substitute face and the rest of the skin comes up. That skin's equalizer window also went dead, because reading the EQ preamp was a missing script capability that aborted the whole script wiring it up. And a skin's spectrum box is now drawn in the colors and opacity the skin asks for: skins that color every band in one stroke were getting bright white bars instead, painted straight across their own artwork — over Rika's song title and time, and over T800's display. Skins that color their bands individually are unchanged.
- **Winamp 5.x modern skins: skins that build themselves from a saved preference come up with their artwork, and their info readouts stop printing on top of each other** — a `.wal` skin can name its background artwork by prefixing a setting of its own (Defix Hi-End 200 does, and never writes a default), so on a first run every one of those names came out empty and the skin lost the wood panelling from the player, both speaker cabinets, the playlist and the library, leaving flat black boxes. A skin asking for artwork that isn't there now keeps what it was already wearing, which is what Winamp shows for an image that failed to load. Text also ignored the transparency a skin sets on it: skins stack several readouts in one slot and fade all but one out, so Kbps, KHz and Channels were being printed on top of each other. That skin's display was empty as well, and its tab strip collapsed into a stack of unreadable stubs at the window's edge, because five things its scripts needed were missing — reading a file's extension, the settings for a per-layer visual effect its analog VU meters use, opening a second copy of one of its own windows, setting a list's text size, and asking whether video is supported — and a skin script that reaches a missing call loses everything it had left to do. It now shows its cassette-deck display with the song title, time, bitrate and format, and its Media Library / Visualization / Explorer tabs are laid out at their proper widths. Eight other reference skins render byte-identically.
- **Winamp 5.x modern skins: the cassette reels turn smoothly and the VU needles follow the music** — the spinning reels and analog needles a `.wal` skin draws started moving in the previous change but did so jerkily, and the meters barely answered what was playing. Neither was the skin's fault: its own animation was keeping perfect time all along. The window was re-scaling every piece of artwork it draws to the display's resolution on **every single frame**, which on a Retina screen cost more than half of the time available for one — that artwork is now scaled once and kept, cutting the cost of a frame by more than five times, and the picture is identical pixel for pixel. On top of that, the playback clock was repainting the whole window ten times a second, which trampled on the animation between its frames; now only the parts that actually follow the clock — the elapsed time, the seek bar — repaint with it. The skin's own rotation maths has also been moved off the drawing step, so a frame is never waiting on it. Separately, the meters were being handed an *average* level where Winamp hands them a *peak*: on ordinary music the average sits in the bottom sixth of a needle's travel, which is exactly how far they were moving. They now get the same peak measurement Winamp uses, taken over the same short window — the audio arrives in chunks of up to a tenth of a second, and a single reading for a whole chunk is nearly the same number every time, so each chunk is now measured in slices and fed to the skin in step with the sound, which is what puts the peaks and dips back. The meters also fall to rest when the music stops: audio measurements simply stop arriving when playback ends, pauses or moves to a speaker on the network, and the needles used to hang wherever the last note left them.

- **Winamp 5.x modern skins: the needles and cassette reels a skin draws now turn** — the analog VU meters and spinning cassette reels in a `.wal` skin are not animation strips: each one is a single still image that Winamp *rotates* on the fly, driven by the skin's own code, and none of that was implemented — so on Defix Hi-End 200 six of its eight display styles sat perfectly still with nothing reporting an error. That per-layer warp now runs, and every style animates. Three further faults were sitting behind it, each hidden by the one in front: a skin starts and stops its animation when its window is shown, and showing a window never told the skin about it (which is also what left the speaker cabinets inert); the needle code called for a square root, which was missing, so its timer aborted on every single tick; and negating a fraction in a skin script threw the fraction away, which left a needle with exactly two positions — parked or pinned — once it did start moving. Skins' scripts can also do the rest of their arithmetic now (powers, logarithms, trigonometry). Meters are fed the same measurement Winamp gives them, so a skin's own calibration behaves as its author intended, and the audio measurement behind it no longer runs on the drawing thread. Repainting is confined to the part of the window that actually changed rather than the whole of it, which is what lets an animation keep time.
- **Winamp 5.x modern skins: the meters a skin draws itself now move, and a skin's own settings are reachable** — every meter a `.wal` skin paints — Defix Hi-End 200's speaker cones, its VU needles, its level bars — reads one call for the level of a given frequency band, and that call was missing, so those parts of a skin were dead no matter what was playing. They are now fed from the same spectrum analysis the rest of nullPlayer's visualizations use. A skin's cover-art panel can also ask whether the artwork is still loading, which was missing too and was aborting the playlist window's repeating script on every tick. **A new Skin Settings window** (Winamp Modern → Skin Settings…) lists the options a skin registers for Winamp's own preferences dialog and often binds no switch of its own to — for Defix that is eight display styles (a cassette deck and seven analog VU meters, all of whose artwork ships in the skin and none of which could be chosen before) and three song-ticker modes, alongside its playlist, notification and appearance options. It shows only what the loaded skin actually registered, applies each change the way the skin's own controls do, and does not appear at all for a skin that registers nothing. **A Skin Windows menu** joins it, for the opposite problem: a skin can declare a whole window and bind no button to it, because in Winamp it is opened from Winamp's own Windows menu — Defix's two speaker cabinets and its configurator were being built and drawn with no way to reach them, as were mmd3's and CornerAmp's colour-theme windows, cPro-Bento's widget manager and T800's Quadhelix window. The menu lists exactly the windows each skin marks as belonging there, and leaves out the ones it marks as private popups or that already have their own menu item.
- **Winamp 5.x modern skins: right-click menus a skin puts on the press, and skin buttons that open the skin's own windows** — a `.wal` skin's buttons can be *assignable*: on Defix Hi-End 200 the four round buttons under the transport each open a window you choose by right-clicking them, and right-clicking them did nothing at all. Winamp gives a skin two separate right-button events, one on the press and one on the release, and only the release was being delivered — that skin puts all four of its menus on the press, so the menus, the artwork and the code behind them all worked and none of them could be opened. Both halves are now delivered, and a menu dismissed away from the button no longer confuses the button it was opened from. Choosing an entry then had to actually open something, and four of the six choices didn't: a skin can open one of its own windows directly from its code rather than by asking the player for it, which until now only flipped an internal flag and left the window shut. It now opens the window — without re-raising one that is already open, since skins make that call on a timer.
...
- **Modern and Metal skins are now named Original and Original-Metal** — the new names appear throughout the skin menus and documentation, while existing preferences, skin folders, and custom `.nsz` bundles remain compatible. The **Modern** name now belongs to the new Winamp 5.x `.wal` skin family described above.
