...
- **New Cava spectrum analyzer window** — a responsive, bar-based audio spectrum visualizer modeled on [cava](https://github.com/karlstav/cava), available from the **Windows** menu (or the main-window right-click menu) next to Flow, in both classic and modern/metal skins. It shows log-spaced frequency bars with gravity/falloff and automatic sensitivity, in **mono** (single row) or **stereo** (mirrored L/R) — double-click the window to toggle. Right-click for a full settings menu: a **Color** submenu with gradient presets (including metallic Gold/Silver/Copper/Bronze/Gunmetal) that by default **follow the active skin's palette** (green for classic), a modern-only **Transparent Background** toggle, and **Bars**, **Smoothing** (responsiveness vs. latency), and **Bass** tilt controls with **Reset to Defaults**. The window docks in the center stack like Spectrum/Flow and scales with UI Size and Hide Title Bars. Audio is analyzed through a dedicated full-rate stereo tap so the full frequency range stays live for both local and streaming playback, and it stays idle (no added CPU) when the window is closed.
- **Cava is now available in the main-window visualization display** — the classic and modern/metal 76×16 analyzer area can render a mono Cava spectrum that follows the active skin palette by default. Its color preset, bar count, smoothing, and bass tilt are independently configurable under **Visuals > Main Window**, without changing the standalone Cava window. The **Visuals** menu is now organized into **Main Window**, **Spectrum Window**, and **Visualizations** submenus; the visualization host is consistently named **Visualizations** across menus, and its engine choice is labeled simply **ProjectM**.
...
- **Classic Library text now follows UI Size** — changing **UI Size** in classic mode now scales Library Browser row text, column headers, bitmap labels, search text, and their row/hit-test metrics, matching Modern and Metal. Manually stretching the Library window still only adds browsing space and does not unexpectedly change its font size.
- **Long-track timers remain readable in every skin mode** — elapsed and remaining times at 100 minutes or more no longer corrupt classic-skin digits or run into the edge of Modern/Metal time panels. Classic skins keep the full `MMM:SS` value and native-size digit art by using the previously unused slot beside the normal minute field; for negative long times, the playback-status icon shifts left to make room for the minus sign. Modern and Metal readouts shrink uniformly only when needed and retain padding on both sides.
- **Tracks dragged into the player can now be cast to Sonos** — dragged lossless files (FLAC/WAV) whose sample rate was not detected at drop time were wrongly rejected as unsupported, failing with *No tracks in the playlist are supported by Sonos* even though Chromecast and library tracks cast correctly. Local lossless files are now probed again before the Sonos compatibility check, while genuinely high-resolution files above 48 kHz remain rejected.
...
- **Playlist stays on the now-playing track when the skin changes** — changing skins during playback (classic↔classic or classic↔modern) made the playlist jump to the top and lose the now-playing highlight, even though audio kept playing from the original slot. The skin change recreates the playlist window, and its reload reset the scroll to slot 0 without re-centering on the current track. Both the classic and modern playlists now restore the selection and scroll back to the actually-playing track after a reload, keeping the UI in sync with playback.
