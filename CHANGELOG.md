...
- **UI Size adds discrete scale choices** — the old **Large UI** toggle is now a **UI Size** submenu with percentage radio choices from **100%** through **200%** in both the Windows menu and the main-window context menu. Size changes apply live in Classic, Modern, and Metal UI; switching between modes while an enlarged size is active restores the exact selected size, and saved sessions now remember the chosen percentage while still restoring older Large UI sessions as 150%.
