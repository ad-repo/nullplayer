...
- **Stream Ripper — download a URL to FLAC/MP3 or a video file** — a new **Output → Streaming → Rip URL…** action opens a dialog where you paste a URL (auto-filled from the clipboard when it holds a web link) and choose an output type: **Audio — FLAC (lossless)**, **Audio — MP3**, or video at a resolution you pick (**720p / 1080p / 1440p / 4K-best**). Ripping shells out to a system-installed `yt-dlp` (+`ffmpeg`); if either isn't found it shows an install hint (`brew install yt-dlp ffmpeg`) rather than failing silently. Quality is prioritized: it grabs the best source streams (`bestaudio` / best video+audio merged), never re-encodes video (the container is remuxed, not transcoded), and for audio does a lossless FLAC encode or top-VBR MP3. The video resolution choice is a hard cap (with codec preference av1 > vp9 > h264 to keep files small) — uncapped "best" can pull multi-gigabyte 4K streams. Output is tagged with the source's metadata (title/artist/album/date) and, for audio, the thumbnail is embedded as cover art; the file is named **`Artist - Title`** from that metadata into a folder you pick. If the source has **chapter timestamps** (common on album/mix uploads), a matching **`.cue` sheet** is written alongside the audio — one TRACK per chapter. Progress shows as a spinner + message band at the top of the main window for the duration of the rip (works in both classic and modern UI). When it finishes, a dialog offers **Play Now** (audio loads into the player; video opens in the video player window, cast-aware), **Reveal in Finder**, or **Done**.
