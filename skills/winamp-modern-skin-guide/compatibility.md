...
window the user closes stays closed for that skin. A `notifier`/`tooltip` container is the one
suppressed case and is recorded in the skin's diagnostics. A browser window is now usable and follows
the ordinary rule. See
