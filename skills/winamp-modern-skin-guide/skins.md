...
| Shield_Amp | **does not load** (B33, 2026-08-23) | nothing — it is the only skin of the 30 installed that fails outright. `WalXML` throws `malformedXML` "Unclosed <container> tag" on `opensource_notifier/notifier.xml`, `<include>`d from `skin.xml:36`: that file opens two `<container>`s, closes one, and ends on `<script file="…"/>`. The skin's own bug, but Winamp loads it and our parser is documented lenient | B33 — nodes attach to their parent at **open** time, so the tree is already complete when `guard stack.isEmpty` fires; warn instead of throwing. Nothing else about the skin has ever been measured |
