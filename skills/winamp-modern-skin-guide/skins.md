...
| impulse | B67, 2026-08-29 | **its Configuration window draws all five of its boxes.** Four declare no `h` and no `relath`, so they resolved to no height and only `Color Themes` — the one that states a height — ever appeared. Each now measures its own body (`autoheightsource` naming the last row, plus the title box's inset), which lands on the 7–9px inter-box gap the skin's sized box has | its own `Impulse:Checkbox` labels clip by ~2 characters — **B68**, an `autowidthsource` defect in the skin's groupdef, not in the Wasabi widgets. Unmeasured beyond the render sweep |
