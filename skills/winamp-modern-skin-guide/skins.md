...
[reference/components/video.md](reference/components/video.md) for why it is a child window and not a subview).
...
(see [reference/components/visualization.md](reference/components/visualization.md) for why this one is a subview where the video
...
|---|---|---|---|
| Love is War Miku | Phase 39 | renders and drives correctly; its visualization window's **Fullscreen / Prev / Next / Menu** and its video window's **Fullscreen / Options** buttons all answer (Phase 39); its `visualizer.maki` menu drives a real oscilloscope and the analyzer's peaks/falloff/coloring (B51) | its video **1x / 2x / TV** buttons are accepted and inert |
...
| Winamp Modern (stock) | Phase 43 | frame, script-built body, playlist + library, **EQ drawer**, **centred title + streaks**; its 22 `VIS_*`/`PE_*`/`VID_*` toolbar buttons answer (Phase 39) — the widest host-action demand in the corpus; its **keyboard shortcuts** answer (Phase 43): `Alt+G` toggles the EQ drawer, `Ctrl+W` shades the focused window, `Alt+A` the album-art window. It is the only skin in the corpus whose key handlers gate on `isActive()` | the 1px title overlay keeps its declared slot; its config drawer's `CB_*` arrows page the thinger's icon set since B34. **Its `Pledit` window was reported not opening once (2026-08-29) and opened normally on the next launch with no fix in between** — not filed, because it never reproduced. What was measured at the time: the catalog answers `playlist=declared:Pledit`, the container renders (436×164, 84 nodes, `PLAYLIST holder (6,40,424,99)`), and the PL button resolves to `TOGGLE guid:pl`, so markup and routing were both correct and the break was downstream, in `routeComponentToggle` → `WinampModernSurfaceCoordinator.toggleSurface` → `setAuxiliaryWindow`. The two fixes in that build (B68, B73) were byte-identical on this skin's `arrangement`/`containers`/`catalog`/`skin windows` lines, so neither explains the recovery. Standing hypothesis, unconfirmed: auxiliary window visibility is persisted and re-asserted on restore, so a recorded-hidden `Pledit` would fight the toggle until a relaunch rewrote it. **If this recurs, capture the persisted record before relaunching** — relaunching is what made it go away — then `WINAMP_MODERN_PLACE_TRACE=1` and click PL |
...
| Itemskin | B69, 2026-08-29 (**confirmed live**); B66; Phase 39 | **its window frames find their content** (B69) — every component window here is a *pair*, a bare box holding the component plus a separate `dynamic="1"` container holding the frame, kept laid over each other by the skin's own `Wasabi:StandardFrame:*` scripts. Both halves read their position as a layout (0) and wrote it as a desktop coordinate, so the frames stayed where the tiler parked them and the contents drew in a column of their own — [skins/itemskin.md](skins/itemskin.md). **Its notifier preferences draw** (B66) — radios, check boxes, three sliders on the skin's own thumbs and two drop-downs, where the window was five empty labelled boxes. Its ten `VIS_*`/`PE_*`/`VID_*` buttons answer (Phase 39). Loads and renders as of Phase 35 — main, mini and Equalizer layouts, the playlist/video/library/AVS windows and the notifier. Its `<include file="xml/eq.xml">` names a file the archive does not ship and is now skipped with a warning | its notifier script wants `getPath` and `setChecked`, which is why its compatibility level reads `unsupported` although the skin draws. Its library frame paints a `basetexture` strip over the left of the hosted library surface and tints the rest (a layer-sizing question, open, found during B69's live QA); window *regions* are still not applied; no `/wal-skin-report` run yet |
| Shield_Amp | B33 (**confirmed live** 2026-08-24) | **loads for the first time** — it was the only skin of the 30 installed that failed outright, and it now renders all nine of its surfaces: `main/normal` and `main/stick`, the equalizer, AVS, Video, MLibrary, the notifier and its Configuration window. The cause was its own bug — `opensource_notifier/notifier.xml`, `<include>`d from `skin.xml:36`, opens two `<container>`s, closes one, and ends on `<script file="…"/>` — but Winamp loads it and our parser is documented lenient, so an unclosed tag at EOF is now a warning (see [reference/loading.md](reference/loading.md) → *What the XML parser tolerates*) | **Its playlist window opens** (B45, **confirmed live** 2026-08-30) — the container was never empty: `xml/pledit.xml` is one `<include file="pledit-normal.xml"/>`, and that file's own `<include file="/standardframe/standardframe.xml"/>` — root-relative, the only such path in the corpus — resolved beside the skin mount instead of inside it, whereupon the missing-include tolerance discarded the *whole* of `pledit-normal.xml` under the outer include's name. Both halves are fixed in [reference/loading.md](reference/loading.md) → *VFS mounts*; `Pledit/normal` now renders 322×200, 67 nodes, with its playlist holder at `(19, 35, 285, 110)`. Its notifier script wants `setChecked`, `setXmlParam` and `urlEncode`, all inert — and `setChecked` is not merely inert, it **aborts `onScriptLoaded`**, so the notifier never initialises at all. That is what keeps this skin from getting B64's track-length readout, which its sibling Styx (identical `opensource_notifier` code) now shows. **Its songticker aborts on a division by zero** (`OneDirectionText.xml`, ×2, found 2026-08-28): the shared `OneDirectionText` widget reads `Config.getItemByGuid("{9149C445-…}").getAttribute("Text Ticker Speed")`, an attribute **no script in this archive ever registers** — the widget is a third-party include expecting a *different* host skin to create it. The unset attribute answers `""`, which is `!= "0"`, so the guard passes and `Delay = 20/stringToFloat("")` divides by zero. Winamp's float divide yields infinity and the handler carries on; ours raises a typed `invalidScript` and abandons the whole `onScriptLoaded`, which is the same "one fault takes the whole handler" shape as an unimplemented method ([reference/harness.md](reference/harness.md) → *The order that made Phase 33 cheap*). Open as **B65**. **Its Configuration window now works** (B14, **confirmed live** 2026-08-29) — the body is one `<Wasabi:TabSheet children="config.stuff;themes.stuff;changelog.stuff">`, and the sheet was what left it an empty slab, not B66. All three tabs draw from the skin's own `wasabi.tabsheet.button.*` artwork and switch, and every form widget inside the three pages **draws and works** — the radios (Effects, Album Cover) and check boxes were dead until B14's QA found that an unbound box drew from its `cfgattrib` provider rather than its own `activated`.  Its **Changelog** page is a `<browser>` pointing at a `file:` URL the navigation policy blocks — unrelated to the sheet, see [reference/components/browser.md](reference/components/browser.md) |
...
| Big Bento Modern ×4 (base, Light, Windows 10 edition, Windows 10 edition Light) | B35, 2026-08-23 | **all four load and render for the first time** — 38 scenes, 80–82 bitmaps in `main/normal`. Three independent causes each fatal on its own: `@SKINSPATH@` was an undefined path variable; the two *Light* editions are **overlays** that pull 6 of their 8 includes out of the base archive (so the base must be installed, under its own name — a lazily mounted sibling, ≤4 per load); and the Windows 10 edition ships a **zero-byte** `window/no_alb_art_shade.png` that failed the whole skin. The Light editions render in their own light palette against the base's artwork — [skins/big-bento-modern.md](skins/big-bento-modern.md) | **B36/B37, 2026-08-23:** five separately reported defects — an overlapping menu bar, a black album-art panel, empty time readouts, a WACUP logo over the Winamp one, a stale search banner — all had **one** cause: 23 `onScriptLoaded` handlers aborted on `System.getSettingsPath()`, which the skin calls near the top of nearly every script to sniff for WACUP. Fixing it surfaced `getAutoHeight` / `getGuid` / `scrollToPercent` behind it. Plus two independent gaps of our own: the `display=` table did not know `TIMEELAPSED` / `SONGLENGTH` / `SONGTITLE` / `SONGSAMPLERATE`, and `offsetx` on a `<text>` was ignored (which is what hides the SUI tab captions in icons-only mode). `instantiate` was still unimplemented — it gated the config pages and the EQ tab and was why the level read `unsupported` (**BB7, 2026-08-23**: implemented as a `GroupList` method that stacks its entries, with `getApplicationPath` behind it; the level is now `degraded`); bitmap overrides in the overlays do not win (see its file). **B38, 2026-08-23:** live QA found five more. The one with reach beyond this skin: `mcvcore` declares `System.onScriptLoaded()` **twice with different bodies**, and the Phase 42 "keep the last binding per (object, event)" rule shadowed the first — so the whole Multi Content View never ran, and the visualization box drew black over the album art. Only a binding whose *body repeats* an earlier one is dropped now. Behind it, the file-info panel's `onSetVisible` cascaded through `getDecoderName` → `getPath` → `getIdealVideoWidth` → `removePath`. The wide-window pane split turned out not to be a defect. **BB7/BB8, 2026-08-23/24 (both confirmed live):** the nine config pages and the SUI equalizer tab are built — each is an empty `<GroupList>` in markup whose content the skin's script expands with `instantiate`, so the EQ tab drew its preset bar and nothing else; and the 77-theme picker applies a theme, through `ColorMgr.getGammaSet(name).apply()` bound by class GUID. Compatibility level is now **degraded**. **BB29, 2026-08-25 (confirmed live):** the left tab bar. Its three modes are a radio group of `cfgattrib`s registered all-`"0"`, and each of the three tab scripts is a three-way `if` with no `else` — so a fresh profile ran none of them, the strip's divider kept its markup `x` of 0 and drew over the icons, and its click was dead because `onLeftClick` only cycles *between* the states. Seeded at load by `WinampModernConfigDefaults` ([reference/loading.md](reference/loading.md)). The "notch" beside each icon was separate: the captions' last pixel column, now not drawn |

| Sony_Walkman | BB2a, 2026-08-25 | its analyzer draws in the grey it asks for. Every band is declared `colorband1="#808589"`, and `#rrggbb` was parsed by a branch left gated off behind `if false` in `8c7e0567` — whose own commit message says it lands that parse — so all 16 bands fell through to `unparseableColor`, i.e. **opaque white bars straight across the SONY wordmark**. See `reference/rendering/colour.md` → *How a colour resolves* | unmeasured beyond the render sweep; its wasabi standard `Component` shell is one of the six containers with no `normal` layout (B26) |

| Ebonite_2_1 | B66, 2026-08-29; BB2a follow-up, 2026-08-25; B15, 2026-08-27 | **its Ebonite Config window is fully populated** (B66) — the heaviest `<Wasabi:CheckBox>` user in the corpus at 48 declarations, across five labelled boxes that were all empty. Its embedded/fallback surfaces were **white text on a near-white panel**. It declares `wasabi.list.background` twice — a `<color>` at 70,70,70 (*"lists/trees item background"*) and a `$solid` bitmap at 237,237,237 (*"Tree background bitmap (tile)"*), the tile last — and its `wasabi.list.text` is white. A real `<color>` now outranks a generated bitmap for colour lookups, so the panel is 70,70,70. Opens on `full` (197×297); it declares no `normal` layout (B26). **Its RGB Color Changer now draws the skin-supplied object frames around the colour swatches, palette, and checkbox controls** (B15, confirmed live 2026-08-27) | its five other layouts are unverified live (see the B26 entry in `TASKS.md`). Its selection row **is readable as of B48** (2026-08-25, confirmed live) — the corpus-wide legibility guard |
| Styx | B66, 2026-08-29 (**radios/check boxes confirmed live**); B64, 2026-08-28 | **its Config window is fully populated and its switches work** (B66; a click on a radio was dead until 2026-08-29 — an unbound box drew from its `cfgattrib` provider's "no" rather than its own `activated`, and *Always on top*, which **is** bound, working beside them is what gave it away): the two boxes that were empty are made of Wasabi standard form widgets — eight `<Wasabi:CheckBox radioid="…">` radios in four sets, a `<Wasabi:HSlider>` that takes the skin's own `wasabi.slider.horizontal.button`, `<Wasabi:Text>` labels, and a `Position` drop-down through its own `Wasabi:CustomDropDownList` wrapper around `<Wasabi:Dropdownlist>`. Its notifier shows the track length again — it formats the duration by hand from `getPlayItemLength()/1000`, which read 0 while the clock answered seconds, so the toast took its own `else` and printed the title alone. Measured before/after in the harness: `settext(… Display    )` → `settext(… Display (4:05))` | only the notifier was measured; the rest of the skin is unmeasured beyond the render sweep. B30's volume-knob verification is still owed (`TASKS.md`) |
| Anexa | B64, 2026-08-28 (**confirmed live**) | **both progress bars fill.** Each is a `<layer>` clipped by a region map and driven only by the skin calling its own `System.onSeek(getPosition())` from a 99 ms timer — a call that had no arity, so the whole handler was abandoned at its first statement. Behind it, `getPosition()`/`getPlayItemLength()` answered *seconds*, and the skin's `int devby = len/255` narrows to 0 there and returns for any track under 4:15. The bar was only ever seen full, because `onStop` sets it to 255. The engine's whole time family is milliseconds now — see [compatibility/maki-surface.md](compatibility/maki-surface.md) → *Time is milliseconds* | the volume/seek pair lives in a slide-out drawer, so the bar is only visible while that drawer is open (as in Winamp). **Its Anexa Config window works** (2026-08-29, confirmed live): one `<Wasabi:TabSheet>` over Colour Themes / Alarm Settings / About, all three tabs switching. The skin ships no `wasabi.tabsheet.button.*` artwork, so the strip is the drawn fallback (B14) |
...
| cPro Insomnis · Insomnis v2 · T2T · das-skin-prev | 2026-08-31 (T2T re-measured against its promo sheet) | all four run the shared ClassicPro engine, so everything cPro-Bento does they do: SUI tabs, drawer + page menu, embedded playlist/EQ/library/video, colour themes. **Their Widgets Manager lists BrowserPro and Now Playing and opens at 313x400** (was a 100x400 sliver), and the Now Playing widget draws its cover | **their titlebar menu bar is deliberately absent** — they ship the template's `(255,0,128)` filler instead of cut artwork and the engine hides the bar itself. Not a defect; see each file. **T2T re-measured to grade C** against the promo sheet that ships *beside* the `.wal`: three engine-level faults — `<images>` unimplemented (volume bar never fills), the SUI tab strip never runs its fit pass at initial layout (4 of 7 tabs unreachable — withdrawn as a harness artifact; the compact classic-player size was blocked by the protective minimum, fixed by B89), and `parser_addCallback` path matching too strict (the skin's 7 custom beat-vis animations never load). All three are engine-level, so **expect the other three to share them** — unswept. **None of the five ships `font.ttf`, and neither does the engine tree**, so every `<truetypefont file="font.ttf">` these skins declare (`cpro.tab.font`, `cpro.songname.font` and six more in `colors.xml`) falls through to the monospaced fallback — do not read a cPro string's width as the skin's intended metrics (B87). Open against them: B78, B82 |
