...
| mmd3 | B32 | text, knobs, drawers, own display all live; **its 82 colour themes list and switch** (in-player drawer + the `ctsbig` window); **its Crossfade / Shuffle / Repeat lamps and display words light**, and Crossfade drives NullPlayer's Sweet Fades (B32) | other `wasabi.*`-backed widgets still draw empty; no scrollbar on the theme list |
