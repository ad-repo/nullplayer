...
| Volume | **Fixed (BB37)** | There was no working volume control in the visible player: the icon click that opens the slider panel, the mute buttons, the play/pause animation and the album-art wiring were all bound to the **shade** layout's objects. Every one of those scripts resolves its targets with `if (normal) {…} if (shade) {…}` over the same variables, and both blocks ran because we build every layout up front — 89 bindings landed in `layout#shade`. `getLayout()` now answers NULL for a layout that has never been shown; see `reference/scripting.md` → *`getLayout()` answers NULL for a layout that has never been shown*. Confirmed live |
