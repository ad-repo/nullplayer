...
**A group's own cut-out is not undone from inside it** (B110, 2026-09-03). The restore above crosses
*groups* — S7Reflex's silhouettes are layers inside `player.normal.drawer` and the `sysregion="1"`
that puts the window back is the sibling `player.main` group declared after it — and that is the only
shape it takes in the corpus. Siblings **inside one group** are describing one shape between them, so
an additive object whose parent also holds a negative one restores nothing.

Ebonite's standard frame is the measured case, and the reason this rule exists. `wasabi.frame.dummy`
carries four `sysregion="-2"` strips that cut the client window down to *exactly* the opening in the
frame window the skin lays over it — top 30, left 10, right 17, bottom 30, that frame's inner rect to
the pixel — and beside them a `sysregion="1"` backdrop layer, `inner`, whose box (`x="11" y="17"
w="-30" h="-20"`, so 11…214 × 17…227 of a 233x230 group) overhangs that opening by 13px into the
title bar and 7px into the bottom bar and falls 19px short on the right. Restored, it printed an
opaque black bar across the frame's own glossy titlebar and under its status row, each stopping short
of the right edge where the backdrop's box ends. Reach, over the 69-skin corpus: one other image
moves, `Shield_Amp/opensource_notifier` by 125 px, and in the same direction — its `notifier.text`
group is `sysregion="1"` and declared after the three `-2` corner silhouettes, so it had been squaring
off the notifier's rounded corners by a pixel of feathering.
