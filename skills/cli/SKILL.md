...
Selecting the library is necessary but not sufficient for **search** on Jellyfin/Emby: `client.search(query:)` used to hit `/Items` with `Recursive=true` and no `parentId`, so it searched the whole server regardless of the selection. `JellyfinManager.search` / `EmbyManager.search` now pass `parentId: currentMusicLibrary?.id`, scoping search to the selected library — consistent with how `fetchArtists`/`fetchAlbums` already filter browsing. This also applies to the GUI Library Browser search (a no-op when the server has a single music library, since that library is auto-selected). Plex search is already library-scoped via `currentLibrary`; Subsonic is music-only.
