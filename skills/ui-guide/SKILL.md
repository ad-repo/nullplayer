...
UI label is **UI Size** with mutually-exclusive percentage rows: **100%**, **105%**, **110%**, **115%**, **125%**, **135%**, **150%**, and **200%**.

- **Scale levels**: `UIScaleLevel` stores percentage raw values (`"100"`, `"105"`, etc.); `scaleFactor` is `percent / 100`.
- **Source of truth**: `WindowManager.uiScaleLevel`; the legacy `isDoubleSize` API remains a compatibility shim (`true` writes 150%, reads are true for any value above 100%).
