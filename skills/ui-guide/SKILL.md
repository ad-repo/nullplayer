...
- **No fallback placement.** There is no top-right / screen-center / "button not available yet" fallback. The reveal is gated on a *settled* anchor (see *Compact window reveal positioning*); if the anchor never resolves, the window stays unrevealed (surfaced loudly in DEBUG) rather than appearing at a guessed spot.
