...
- **Streaming URL content type (Sonos)**: Subsonic stream URLs (`/rest/stream?id=...`) have no file extension, so `detectAudioContentType(for:)` defaults to `audio/mpeg`. This breaks Sonos casting for non-MP3 formats. Prefer `Track.contentType` set by the server client from API metadata.
