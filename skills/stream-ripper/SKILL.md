...
Common: `--no-playlist --embed-metadata`, output template `"<local staging>/%(artist|)s%(artist& - |)s%(title)s.%(ext)s"` (→ `Artist - Title.ext`, or just `Title.ext` when no artist), and `--print-to-file after_move:filepath <tmp>` so the **actual** final path (extension depends on the native codec) is read back. The completed local file is copied to a hidden temporary sibling on the destination volume and renamed into place only after the copy completes. Existing files are preserved with a collision-free name, and failed final transfers retain the completed local staging file for recovery.
...
`readChapters` / `writeCueFile` / `cueTimestamp` are `nonisolated static` so they run safely in the background completion block. `writeCueFile` builds the cue in local staging and sends it through the same destination-volume temporary-copy commit as the media file; it reports transfer errors instead of claiming a cue was written.
...
- During the final transfer, a hidden `.nullplayer-transfer-*` file can briefly appear in the destination folder. It is the completed-file copy being written before the same-volume commit and is removed if the transfer fails.
