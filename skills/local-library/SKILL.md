...
For non-local file volumes, direct local playback (`loadLocalTrackForImmediatePlayback`) and gapless pre-scheduling (`scheduleNextTrackForGapless`) stage the audio file into the system temp directory before scheduling it. This avoids `AVAudioPlayerNode` render/pre-fetch reads going back to SMB/NFS during playback or seek. The copy policy is disk-space based, not file-size capped: copy when the temp volume can hold the file while keeping a 1 GiB reserve; otherwise fall back to opening the original NAS path. Temp copies use the `nullplayer-` filename prefix, are removed on track replacement/teardown, and are cleaned on `AudioEngine` launch to recover files left behind by crashes or force-quits.

> ⚠️ **Side effect — the launch cleanup deletes _all_ `nullplayer-`-prefixed temp files.** `cleanupStaleTemporaryPlaybackCopies()` (run once in `AudioEngine.init`) removes every regular file in the system temp directory whose name starts with `nullplayer-`, not just staged playback copies. Other subsystems share that prefix — StreamRipper (`nullplayer-yt-*`, `nullplayer-rip-*`, `nullplayer-chapters-*`) and YouTubeManager (`nullplayer-ytdlp-*`). This is harmless within a single process (the sweep runs before any rip/download starts, so it only clears stale leftovers), but a **second concurrent instance** — the headless CLI while the GUI app is running, or `open -n` — will delete an in-progress rip/download's capture files. **For users:** avoid ripping/downloading in one instance while launching another. **For agents:** if you add a new `nullplayer-`-prefixed temp file that must survive a concurrent launch, give it a prefix that does NOT begin with the `temporaryPlaybackCopyPrefix` string, or narrow the sweep's match to a playback-specific prefix (e.g. `nullplayer-playbackcopy-`).
