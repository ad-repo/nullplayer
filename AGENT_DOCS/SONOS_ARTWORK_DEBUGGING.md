...
When casting audio to Sonos from NullPlayer, the artwork does not appear in the Sonos app, even though:
...
**Files modified**:
- `Sources/NullPlayerCore/Models/Track.swift` - Added `artworkThumb: String?` property
- `Sources/NullPlayer/Data/Models/Track.swift` - Added `artworkThumb: String?` property  
- `Sources/NullPlayer/Plex/PlexManager.swift` - Pass `plexTrack.thumb` to `artworkThumb` in `convertToTrack()`
- `Sources/NullPlayer/Subsonic/SubsonicManager.swift` - Pass `song.coverArt` to `artworkThumb` in `convertToTrack()`
- `Sources/NullPlayer/Casting/CastManager.swift` - Created `getArtworkURL(for:)` helper, removed dead `findPlexTrack()`
- `Sources/NullPlayer/Audio/AudioEngine.swift` - Preserve `artworkThumb` when recreating Track
...
**Files modified**:
- `Sources/NullPlayer/Casting/CastDevice.swift` - Rewrote `toDIDLLite()` to use string concatenation
...
**Files modified**:
- `Sources/NullPlayer/Plex/PlexServerClient.swift` - Added `directArtworkURL(thumb:)` method
- `Sources/NullPlayer/Plex/PlexManager.swift` - Added `directArtworkURL(thumb:)` wrapper
- `Sources/NullPlayer/Casting/CastManager.swift` - Changed to use `directArtworkURL()`
...
**Files modified**:
- `Sources/NullPlayer/Casting/CastDevice.swift` - Added DLNA namespace, added `dlna:profileID="JPEG_TN"` attribute
...
**Files modified**:
- `Sources/NullPlayer/Plex/PlexServerClient.swift` - Modified `directArtworkURL()` to use transcode endpoint with manual URL construction
...
**Files modified**:
- `Sources/NullPlayer/Casting/LocalMediaServer.swift` - Added `registerRemoteArtwork(from:)` method that:
  1. Fetches image from remote URL (Plex/Subsonic)
  2. Stores in memory with unique token
  3. Returns simple local HTTP URL
- `Sources/NullPlayer/Casting/CastManager.swift` - Changed `getArtworkURL()` to proxy through LocalMediaServer
...
**Files modified**:
- `Sources/NullPlayer/Casting/CastDevice.swift` - Removed `dlna:profileID` attribute
...
**Files modified**:
- `Sources/NullPlayer/Casting/CastDevice.swift` - Reordered elements, removed DLNA namespace
...
- `Sources/NullPlayer/Casting/CastDevice.swift` - DIDL-Lite generation changes
- `Sources/NullPlayer/Casting/CastManager.swift` - getArtworkURL with LocalMediaServer proxy
- `Sources/NullPlayer/Casting/LocalMediaServer.swift` - registerRemoteArtwork method
- `Sources/NullPlayer/Plex/PlexServerClient.swift` - directArtworkURL method
- `Sources/NullPlayer/Plex/PlexManager.swift` - directArtworkURL wrapper
- `Sources/NullPlayer/Casting/UPnPManager.swift` - Debug logging (artworkURL, DIDL-Lite)

The following files have working changes that should be KEPT:
- `Sources/NullPlayerCore/Models/Track.swift` - artworkThumb property
- `Sources/NullPlayer/Data/Models/Track.swift` - artworkThumb property
